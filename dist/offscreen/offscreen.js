...
// src/offscreen/download.js
var PART_SUFFIX = ".part", verifiedThisDocument = /* @__PURE__ */ new Set(), retainedVerifiedBytes = null;
...
async function modelsDir() {
  return (await navigator.storage.getDirectory()).getDirectoryHandle("models", { create: !0 });
}
...
async function getPartialDownloadProgress(entry) {
  if (entry.bytes == null || entry.bytes <= 0) return 0;
  let dir = await modelsDir(), size = await partialSize(dir, entry.id + PART_SUFFIX);
  return Math.min(size / entry.bytes, 1);
}
async function verifyAndInstall(entry, dir, partHandle) {
  let partFile = await partHandle.getFile();
  if (entry.bytes != null && partFile.size !== entry.bytes)
    throw partFile.size > entry.bytes && await dir.removeEntry(entry.id + PART_SUFFIX).catch(() => {
    }), new Error(
      `model '${entry.id}' download incomplete: expected ${entry.bytes} bytes, got ${partFile.size}`
    );
  let actual = await sha256Hex(await partFile.arrayBuffer());
  if (actual !== entry.sha256?.toLowerCase())
    throw await dir.removeEntry(entry.id + PART_SUFFIX).catch(() => {
    }), new Error(
      `model '${entry.id}' hash mismatch: expected ${entry.sha256}, got ${actual}. Partial file deleted; download will restart from zero.`
    );
  await /** @type {{move: (name: string) => Promise<void>}} */
  /** @type {unknown} */
  partHandle.move(entry.id);
  let final = await (await dir.getFileHandle(entry.id)).getFile();
  return verifiedThisDocument.add(verificationKey(entry)), final;
}
function contentRange(value) {
  let match = value?.trim().match(/^bytes\s+(\d+)-(\d+)\/(\d+|\*)$/i);
  return match ? {
...
function unsatisfiedRangeTotal(value) {
  let match = value?.trim().match(/^bytes\s+\*\/(\d+)$/i);
  return match ? Number(match[1]) : null;
}
async function getInstalledModel(entry) {
  try {
    let dir = await modelsDir(), file = await (await dir.getFileHandle(entry.id)).getFile(), key = verificationKey(entry);
    if (entry.bytes != null && file.size !== entry.bytes)
      return verifiedThisDocument.delete(key), await dir.removeEntry(entry.id).catch(() => {
      }), null;
    if (entry.sha256 && !verifiedThisDocument.has(key)) {
      let buf = await file.arrayBuffer();
      if (await sha256Hex(buf) !== entry.sha256.toLowerCase())
        return await dir.removeEntry(entry.id).catch(() => {
        }), null;
      verifiedThisDocument.add(key), retainedVerifiedBytes = buf;
    }
    return file;
  } catch {
    return null;
...
async function getInstalledModelBytes(entry) {
  let file = await getInstalledModel(entry);
  if (!file) return null;
  if (retainedVerifiedBytes) {
    let bytes = retainedVerifiedBytes;
    return retainedVerifiedBytes = null, bytes;
  }
  return file.arrayBuffer();
}
async function downloadAndInstall(entry, onProgress) {
  if (!entry.url || !entry.sha256)
    throw new Error(`model '${entry.id}' has no pinned url/sha256 yet (models/manifest.json)`);
  let installed = await getInstalledModel(entry);
  if (installed) return installed;
  let dir = await modelsDir(), partName = entry.id + PART_SUFFIX, offset = await partialSize(dir, partName);
  entry.bytes != null && offset > entry.bytes && (await dir.removeEntry(partName).catch(() => {
  }), offset = 0);
  let highestProgress = 0, report = (written, total) => {
    !onProgress || total <= 0 || (highestProgress = Math.max(highestProgress, Math.min(written / total, 1)), onProgress(highestProgress));
  };
  if (report(offset, entry.bytes ?? 0), entry.bytes != null && offset === entry.bytes) {
    let completePart = await dir.getFileHandle(partName);
    return verifyAndInstall(entry, dir, completePart);
  }
  for (let attempt = 0; attempt < 2; attempt += 1) {
    let headers = offset > 0 ? { Range: `bytes=${offset}-` } : {}, res = await fetch(entry.url, { headers, credentials: "omit", cache: "no-store" });
    if (offset > 0 && res.status === 416) {
      let remoteTotal = unsatisfiedRangeTotal(res.headers.get("Content-Range"));
      if (remoteTotal != null && offset === remoteTotal) {
        let completePart = await dir.getFileHandle(partName);
        return verifyAndInstall(entry, dir, completePart);
      }
      if (await res.body?.cancel().catch(() => {
      }), await dir.removeEntry(partName).catch(() => {
      }), offset = 0, attempt === 0) continue;
    }
    if (!res.ok && res.status !== 206)
      throw new Error(`model download failed: HTTP ${res.status}`);
    if (!res.body) throw new Error("model download failed: empty body");
    let range = contentRange(res.headers.get("Content-Range"));
    if (res.status === 206 && (!range || range.start !== offset))
      throw await res.body.cancel().catch(() => {
      }), new Error(
        `model download failed: invalid Content-Range for offset ${offset}: ${res.headers.get("Content-Range") ?? "missing"}`
      );
    offset > 0 && res.status === 200 && (offset = 0);
    let partHandle = await dir.getFileHandle(partName, { create: !0 }), writable = await partHandle.createWritable({ keepExistingData: offset > 0 });
    await writable.seek(offset);
    let declaredLength = Number(res.headers.get("Content-Length") ?? 0), total = entry.bytes ?? range?.total ?? declaredLength + offset, written = offset;
    ...
    try {
      for (; ; ) {
        ...
        if (done) break;
        await writable.write(value), written += value.byteLength, report(written, total);
      }
      await writable.close();
    } catch (err) {
      throw await writable.close().catch(() => {
      }), err;
    } finally {
      reader.releaseLock();
    }
...
// src/offscreen/webgpu-windows.js
...
function stripPowerPreference(options) {
  if (!options || typeof options != "object" || !Object.prototype.hasOwnProperty.call(options, "powerPreference"))
    return options;
  let rest = { .../** @type {Record<string, unknown>} */
  options };
  return delete rest.powerPreference, Object.keys(rest).length > 0 ? rest : void 0;
}
function wrapRequestAdapter(target) {
  if (wrapped.has(target) || typeof target.requestAdapter != "function") return !1;
  let original = target.requestAdapter;
  ...
  return wrapped.add(target), !0;
}
function silenceWindowsWebGpuPowerPreference(gpu, userAgent) {
  let agent = userAgent ?? globalThis.navigator?.userAgent ?? "";
  if (!/Windows/i.test(agent)) return !1;
  let did = !1, target = gpu ?? /** @type {{gpu?: GpuLike} | undefined} */
  globalThis.navigator?.gpu;
  return target && (did = wrapRequestAdapter(target) || did), did;
}
 ...
 */
var Hr = Object.defineProperty, jf = Object.getOwnPropertyDescriptor, Hf = Object.getOwnPropertyNames, qf = Object.prototype.hasOwnProperty, qr = ((a) => typeof __require < "u" ? __require : typeof Proxy < "u" ? new Proxy(a, { get: (r, s) => (typeof __require < "u" ? __require : r)[s] }) : a)(function(a) {
  if (typeof __require < "u") return __require.apply(this, arguments);
  throw Error('Dynamic require of "' + a + '" is not supported');
}), k = (a, r) => () => (a && (r = a(a = 0)), r), At = (a, r) => {
  for (var s in r) Hr(a, s, { get: r[s], enumerable: !0 });
}, Yf = (a, r, s, f) => {
  if (r && typeof r == "object" || typeof r == "function") for (let i of Hf(r)) !qf.call(a, i) && i !== s && Hr(a, i, { get: () => r[i], enumerable: !(f = jf(r, i)) || f.enumerable });
  return a;
}, zt = (a) => Yf(Hr({}, "__esModule", { value: !0 }), a), Vt, Ke, Qe, Jf, va, Yr = k(() => {
  "use strict";
  Vt = /* @__PURE__ */ new Map(), Ke = [], Qe = (a, r, s) => {
    if (r && typeof r.init == "function" && typeof r.createInferenceSessionHandler == "function") {
      let f = Vt.get(a);
      if (f === void 0) Vt.set(a, { backend: r, priority: s });
      else {
      ...
      if (s >= 0) {
        let i = Ke.indexOf(a);
        i !== -1 && Ke.splice(i, 1);
        for (let d = 0; d < Ke.length; d++) if (Vt.get(Ke[d]).priority <= s) {
          Ke.splice(d, 0, a);
          return;
        }
        Ke.push(a);
    ...
    if (r.aborted) return r.error;
    {
      let s = !!r.initPromise;
      try {
        return s || (r.initPromise = r.backend.init(a)), await r.initPromise, r.initialized = !0, r.backend;
      } catch (f) {
        return s || (r.error = `${f}`, r.aborted = !0), r.error;
      } finally {
        delete r.initPromise;
      }
    }
  }, va = async (a) => {
    let r = a.executionProviders || [], s = r.map((y) => typeof y == "string" ? y : y.name), f = s.length === 0 ? Ke : s, i, d = [], l = /* @__PURE__ */ new Set();
    for (let y of f) {
      let w = await Jf(y);
  ...
  "use strict";
  Aa(), Ia = "warning", ie = { wasm: {}, webgl: {}, webgpu: {}, versions: { common: Sa }, set logLevel(a) {
    if (a !== void 0) {
    ...
    return Ia;
  } }, Object.defineProperty(ie, "logLevel", { enumerable: !0 });
}), Q, xa = k(() => {
  "use strict";
  Jr(), Q = ie;
}), La, Ba, Oa = k(() => {
  ...
  La = (a, r) => {
    ...
    if (f != null) {
      ...
      l === "RGBA" ? (g = 0, v = T, S = T * 2, U = T * 3) : l === "RGB" ? (g = 0, v = T, S = T * 2) : l === "RBG" && (g = 0, S = T, v = T * 2);
      for (let R = 0; R < d; R++) for (let H = 0; H < i; H++) {
        let C = (a.data[g++] - w[0]) * y[0], M = (a.data[v++] - w[1]) * y[1], q = (a.data[S++] - w[2]) * y[2], B = U === -1 ? 255 : (a.data[U++] - w[3]) * y[3];
        f.fillStyle = "rgba(" + C + "," + M + "," + q + "," + B + ")", f.fillRect(H, R, 1, 1);
      }
    ...
    if (s != null) {
      ...
      let v = 4, S = 0, U = 1, R = 2, H = 3, C = 0, M = g, q = g * 2, B = -1;
      m === "RGBA" ? (C = 0, M = g, q = g * 2, B = g * 3) : m === "RGB" ? (C = 0, M = g, q = g * 2) : m === "RBG" && (C = 0, q = g, M = g * 2), f = s.createImageData(i, d);
      for (let W = 0; W < d * i; S += v, U += v, R += v, H += v, W++) f.data[S] = (a.data[C++] - T[0]) * w[0], f.data[U] = (a.data[M++] - T[1]) * w[1], f.data[R] = (a.data[q++] - T[2]) * w[2], f.data[H] = B === -1 ? 255 : (a.data[B++] - T[3]) * w[3];
    } else throw new Error("Can not access image data");
    return f;
  };
}), Xr, Ma, Ua, Ca, Da, Pa, _a = k(() => {
  "use strict";
  jt(), Xr = (a, r) => {
    ...
    let m = r.format !== void 0 ? r.format : "RGBA", y = r.tensorFormat !== void 0 && r.tensorFormat !== void 0 ? r.tensorFormat : "RGB", w = s * f, T = y === "RGBA" ? new Float32Array(w * 4) : new Float32Array(w * 3), g = 4, v = 0, S = 1, U = 2, R = 3, H = 0, C = w, M = w * 2, q = -1;
    m === "RGB" && (g = 3, v = 0, S = 1, U = 2, R = -1), y === "RGBA" ? q = w * 3 : y === "RBG" ? (H = 0, M = w, C = w * 2) : y === "BGR" && (M = 0, C = w, H = w * 2);
    for (let W = 0; W < w; W++, v += g, U += g, S += g, R += g) T[H++] = (a[v] + l[0]) / d[0], T[C++] = (a[S] + l[1]) / d[1], T[M++] = (a[U] + l[2]) / d[2], q !== -1 && R !== -1 && (T[q++] = (a[R] + l[3]) / d[3]);
    ...
    if (s) {
      ...
      if (g != null) {
        let v = a.height, S = a.width;
        return g.drawImage(a, 0, 0, S, v), l = g.getImageData(0, 0, S, v).data, m.height = v, m.width = S, Xr(l, m);
      } else throw new Error("Can not access image data");
    } else {
      if (d) return new Promise((T, g) => {
        ...
        U.crossOrigin = "Anonymous", U.src = a, U.onload = () => {
          ...
          m.height = v.height, m.width = v.width, T(Xr(R.data, m));
        };
      });
      throw new Error("Input data provided is not supported - aborted tensor creation");
    }
    if (l !== void 0) return Xr(l, m);
    throw new Error("Input data provided is not supported - aborted tensor creation");
  }, Ua = (a, r) => {
    ...
    async run(r, s, f) {
      ...
      if (typeof s == "object") {
        ...
        if (Array.isArray(s)) {
          ...
          let w = !1, T = Object.getOwnPropertyNames(s);
          for (let g of this.outputNames) if (T.indexOf(g) !== -1) {
            let v = s[g];
            (v === null || v instanceof Le) && (w = !0, l = !1, i[g] = v);
          }
          if (w) {
            if (typeof f == "object" && f !== null) d = f;
            else if (typeof f < "u") throw new TypeError("'options' must be an object.");
...
At(Qr, { InferenceSession: () => Xf, TRACE: () => $a, TRACE_EVENT_BEGIN: () => $e, TRACE_EVENT_END: () => ze, TRACE_FUNC_BEGIN: () => tt, TRACE_FUNC_END: () => rt, Tensor: () => Le, env: () => Q, registerBackend: () => Qe });
var Ve = k(() => {
  "use strict";
  Ea(), xa(), ja(), Zr(), Ha(), qa(), Kr(), Ya(), Ja();
}), qt = k(() => {
  "use strict";
}), Qa = {};
At(Qa, { default: () => Zf });
var Za, Ka, Zf, es = k(() => {
  "use strict";
  en(), je(), Yt(), Za = "ort-wasm-proxy-worker", Ka = globalThis.self?.name === Za, Ka && (self.onmessage = (a) => {
    ...
    try {
      switch (r) {
        ...
        case "release":
          Qt(s), postMessage({ type: r });
          break;
        case "run": {
          let { sessionId: f, inputIndices: i, inputs: d, outputIndices: l, options: m } = s;
          er(f, i, d, l, new Array(l.length).fill(null), m).then((y) => {
            y.some((w) => w[3] !== "cpu") ? postMessage({ type: r, err: "Proxy does not support non-cpu tensor location." }) : postMessage({ type: r, out: y }, rr([...d, ...y]));
          }, (y) => {
...
async function ts(a = {}) {
  ...
  var g, v, S, U, R, H, C = console.log.bind(console), M = console.error.bind(console), q = C, B = M, W = !1, oe = (e) => e.startsWith("file://");
  function p() {
    ...
    let e = function(t) {
      try {
        ...
        if (o === "load") {
          ...
          for (let c of n.ee) r[c] && !r[c].proxy || (r[c] = (...h) => {
            postMessage({ Pc: "callHandler", de: c, args: h });
          }, c == "print" && (q = r[c]), c == "printErr" && (B = r[c]));
          Fe = n.ke, se(), v = n.le, bt(), $t();
        } else if (o === "run") {
          (function(u) {
            var c = (p(), A)[u + 52 >>> 2 >>> 0];
            u = (p(), A)[u + 56 >>> 2 >>> 0], Uo(c, c - u), D(c);
          })(n.Oc), Fr(n.Oc, 0, 0, 1, 0, 0), bn(), Ir(n.Oc), ne || (po(), ne = !0);
          try {
            zs(n.ie, n.Xc);
          } catch (u) {
            if (u != "unwind") throw u;
          }
        } else n.target !== "setimmediate" && (o === "checkMailbox" ? ne && Pt() : o && (B(`worker: received unknown command ${o}`), B(n)));
      } catch (u) {
        throw xo(), u;
      }
    };
  ...
  async function bt() {
    ...
    return r.instantiateWasm ? new Promise((o) => {
    ...
    }) : i ? e(new WebAssembly.Instance(v, qe()), v) : (we ??= r.locateFile ? r.locateFile ? r.locateFile("ort-wasm-simd-threaded.asyncify.wasm", T) : T + "ort-wasm-simd-threaded.asyncify.wasm" : new URL("ort-wasm-simd-threaded.asyncify.wasm", import.meta.url).href, t = await (async function(o) {
      var u = we;
      if (!g && !oe(u)) try {
      ...
      return (async function(h, b) {
        try {
          ...
          B(`failed to asynchronously prepare wasm: ${I}`), Te(I);
        }
      })(u, o);
  ...
  var Se = (e) => {
    ...
    var t = We.pop();
    if (!t) return 6;
    gt.push(t), Je[e.Oc] = t, t.Oc = e.Oc;
    var n = { Pc: "run", ie: e.he, Xc: e.Xc, Oc: e.Oc };
    return t.postMessage(n, e.Zc), 0;
  }, G = 0, V = (e, t, ...n) => {
    var o, u = 16 * n.length, c = P(), h = Gt(u), b = h >>> 3;
  ...
  var wr = (e) => {
    if (S = e, i) throw br(e), "unwind";
    Ye(e);
  }, We = [], gt = [], hn = [], Je = {}, yn = (e) => {
    var t = e.Oc;
    delete Je[t], We.push(e), gt.splice(gt.indexOf(e), 1), e.Oc = 0, Bo(t);
  };
  function bn() {
    hn.forEach((e) => e());
  }
  var wn = (e) => new Promise((t) => {
    e.onmessage = (u) => {
      var c = u.data;
      if (u = c.Pc, c.Wc && c.Wc != Ft()) {
        var h = Je[c.Wc];
        h ? h.postMessage(c, c.Zc) : B(`Internal error! Worker sent a message "${u}" to target pthread ${c.Wc}, but that thread no longer exists!`);
      } else u === "checkMailbox" ? Pt() : u === "spawnThread" ? K(c) : u === "cleanupThread" ? he(() => {
        yn(Je[c.je]);
      }) : u === "loaded" ? (e.loaded = !0, t(e)) : c.target === "setimmediate" ? e.postMessage(c) : u === "uncaughtException" ? e.onerror(c.error) : u === "callHandler" ? r[c.de](...c.args) : u && B(`worker sent an unknown command ${u}`);
    }, e.onerror = (u) => {
      throw B(`worker sent an error! ${u.filename}:${u.lineno}: ${u.message}`), u;
    };
    var n, o = [];
    for (n of []) r.propertyIsEnumerable(n) && o.push(n);
    e.postMessage({ Pc: "load", ee: o, ke: Fe, le: v });
  });
  function gn() {
    var e = new Worker((() => {
      let t = URL;
  ...
  var Fe, zs = (e, t) => {
    G = 0, e = Vr(e, t), 0 < G ? S = e : Gr(e);
  }, Ct = [], Dt = 0, ce = (e) => -9007199254740992 > e || 9007199254740992 < e ? NaN : Number(e);
  function Vs(e) {
    var t = new gr(e >>>= 0);
    return (p(), X)[t.Rc + 12 >>> 0] == 0 && (Tn(t, !0), Dt--), vn(t, !1), Ct.push(t), Po(e);
  }
  var ft = 0, js = () => {
    N(0, 0);
    var e = Ct.pop();
  ...
  function Tn(e, t) {
    t = t ? 1 : 0, (p(), X)[e.Rc + 12 >>> 0] = t;
  }
  function vn(e, t) {
    t = t ? 1 : 0, (p(), X)[e.Rc + 13 >>> 0] = t;
  }
  class gr {
    constructor(t) {
      this.Yc = t, this.Rc = t - 24;
    }
  }
  var Tr = (e) => {
    var t = ft;
    ...
    var n = new gr(t);
    (p(), A)[n.Rc + 16 >>> 2 >>> 0] = t;
    var o = (p(), A)[n.Rc + 4 >>> 2 >>> 0];
    if (!o) return St(0), t;
    for (var u of e) {
      if (u === 0 || u === o) break;
      if (Do(u, o, n.Rc + 16)) return St(u), t;
    }
    return St(o), t;
  };
  function Hs() {
  ...
  function Ys(e, t, n, o) {
    return Tr([e >>> 0, t >>> 0, n >>> 0, o >>> 0]);
  }
  var Js = () => {
    var e = Ct.pop();
    e || Te("no exception to throw");
    var t = e.Yc;
    throw (p(), X)[e.Rc + 13 >>> 0] == 0 && (Ct.push(e), vn(e, !0), Tn(e, !1), Dt++), zr(t), ft = t;
  };
  function Xs(e, t, n) {
    var o = new gr(e >>>= 0);
    throw t >>>= 0, n >>>= 0, (p(), A)[o.Rc + 16 >>> 2 >>> 0] = 0, (p(), A)[o.Rc + 4 >>> 2 >>> 0] = t, (p(), A)[o.Rc + 8 >>> 2 >>> 0] = n, zr(e), Dt++, ft = e;
  }
  var Zs = () => Dt;
  function En(e, t, n, o) {
  ...
  function Sn(e, t, n, o) {
    ...
    var u = [];
    return i && u.length === 0 ? En(e, t, n, o) : (e = { he: n, Oc: e, Xc: o, Zc: u }, i ? (e.Pc = "spawnThread", postMessage(e, u), 0) : K(e));
  }
  function Ks(e) {
    throw ft ||= e >>> 0, ft;
  }
  var An = globalThis.TextDecoder && new TextDecoder(), In = (e, t, n, o) => {
    ...
    if (16 < (n = In(e, t >>>= 0, n, o)) - t && e.buffer && An) return An.decode(e.buffer instanceof ArrayBuffer ? e.subarray(t, n) : e.slice(t, n));
    for (o = ""; t < n; ) {
      var u = e[t++];
      if (128 & u) {
        var c = 63 & e[t++];
  ...
  function Bn(e, t) {
    if (i) return V(4, 1, e, t);
  }
  function On(e, t) {
    if (i) return V(5, 1, e, t);
  }
  function Mn(e, t, n) {
    if (i) return V(6, 1, e, t, n);
  }
  function Un(e, t, n) {
    return i ? V(7, 1, e, t, n) : 0;
  }
  function Cn(e, t) {
    if (i) return V(8, 1, e, t);
  }
  function Dn(e, t, n) {
    if (i) return V(9, 1, e, t, n);
  }
  function Pn(e, t, n, o) {
    if (i) return V(10, 1, e, t, n, o);
  }
  function _n(e, t, n, o) {
    if (i) return V(11, 1, e, t, n, o);
  }
  function Rn(e, t, n, o) {
    if (i) return V(12, 1, e, t, n, o);
  }
  function Nn(e) {
    if (i) return V(13, 1, e);
  }
  function kn(e, t) {
    if (i) return V(14, 1, e, t);
  }
  function Wn(e, t, n) {
  ...
  }, vr = {}, Er = {}, ei = {}, dt = class extends Error {
    constructor(e) {
  ...
  function De(e, t, n = {}) {
    return (function(o, u, c = {}) {
      ...
      if (Er.hasOwnProperty(o)) {
        if (c.fe) return;
        throw new dt(`Cannot register type '${h}' twice`);
      }
      Er[o] = u, delete ei[o], vr.hasOwnProperty(o) && (u = vr[o], delete vr[o], u.forEach((b) => b()));
    })(e, t, n);
  }
  var Fn = (e, t, n) => {
    switch (t) {
      ...
      case 2:
        return n ? (o) => (p(), Ce)[o >>> 1 >>> 0] : (o) => (p(), Z)[o >>> 1 >>> 0];
      case 4:
        return n ? (o) => (p(), x)[o >>> 2 >>> 0] : (o) => (p(), A)[o >>> 2 >>> 0];
      case 8:
        return n ? (o) => (p(), me)[o >>> 3 >>> 0] : (o) => (p(), Y)[o >>> 3 >>> 0];
      default:
  ...
  function ti(e, t, n, o, u) {
    ...
    De(e, { name: t, Nc: c, Tc: (h, b) => (typeof b == "number" && (b = BigInt(b)), b), Sc: Fn(t, n, !o), Uc: null });
  }
  function ri(e, t, n, o) {
    De(e >>>= 0, { name: t = Oe(t >>> 0), Nc: function(u) {
      return !!u;
    }, Tc: function(u, c) {
      return c ? n : o;
    }, Sc: function(u) {
      return this.Nc((p(), J)[u >>> 0]);
    }, Uc: null });
  }
  var Gn = [], Xe = [0, 1, , 1, null, 1, !0, 1, !1, 1];
  function Sr(e) {
  ...
  function Ar(e) {
    return this.Nc((p(), A)[e >>> 2 >>> 0]);
  }
  var ni = { name: "emscripten::val", Nc: (e) => {
    var t = ve(e);
    return Sr(e), t;
  }, Tc: (e, t) => Ie(t), Sc: Ar, Uc: null };
  function oi(e) {
    return De(e >>> 0, ni);
  }
  var ai = (e, t) => {
    switch (t) {
      case 4:
        return function(n) {
          return this.Nc((p(), _)[n >>> 2 >>> 0]);
        };
      case 8:
        return function(n) {
          return this.Nc((p(), ae)[n >>> 3 >>> 0]);
        };
      default:
  ...
  function si(e, t, n) {
    n >>>= 0, De(e >>>= 0, { name: t = Oe(t >>> 0), Nc: (o) => o, Tc: (o, u) => u, Sc: ai(t, n), Uc: null });
  }
  function ii(e, t, n, o, u) {
    ...
    De(e, { name: t, Nc: c, Tc: (b, E) => E, Sc: Fn(t, n, o !== 0), Uc: null });
  }
  function ui(e, t, n) {
    function o(c) {
      var h = (p(), A)[c >>> 2 >>> 0];
    ...
    De(e >>>= 0, { name: n = Oe(n >>> 0), Nc: o, Sc: o }, { fe: !0 });
  }
  var Pe = (e, t, n) => {
    var o = (p(), J);
    if (t >>>= 0, 0 < n) {
  ...
  function fi(e, t) {
    De(e >>>= 0, { name: t = Oe(t >>> 0), Nc(n) {
      var o = (p(), A)[n >>> 2 >>> 0];
      return o = ct(n + 4, o, !0), xe(n), o;
    }, Tc(n, o) {
  ...
  var $n = globalThis.TextDecoder ? new TextDecoder("utf-16le") : void 0, ci = (e, t, n) => {
    if (e >>>= 1, 16 < (t = In((p(), Z), e, t / 2, n)) - e && $n) return $n.decode((p(), Z).slice(e, t));
    for (n = ""; e < t; ++e) {
      var o = (p(), Z)[e >>> 0];
  ...
  function yi(e, t, n) {
    if (e >>>= 0, t >>>= 0, n = Oe(n >>>= 0), t === 2) var o = ci, u = di, c = li;
    else o = pi, u = mi, c = hi;
    De(e, { name: n, Nc: (h) => {
      var b = (p(), A)[h >>> 2 >>> 0];
      return b = o(h + 4, b * t, !0), xe(h), b;
    }, Tc: (h, b) => {
  ...
  function bi(e, t) {
    De(e >>>= 0, { ge: !0, name: t = Oe(t >>> 0), Nc: () => {
    }, Tc: () => {
    } });
  }
  function wi(e) {
    Fr(e >>> 0, !f, 1, !s, 131072, !1), bn();
  }
  var he = (e) => {
    if (!W) try {
      if (e(), !(0 < G)) try {
  ...
  function Ir(e) {
    e >>>= 0, gi || (Atomics.waitAsync((p(), x), e >>> 2, e).value.then(Pt), e += 128, Atomics.store((p(), x), e >>> 2, 1));
  }
  var Pt = () => he(() => {
    var e = Ft();
    e && (Ir(e), Mo());
  ...
  function Ti(e, t) {
    (e >>>= 0) == t >>> 0 ? setTimeout(Pt) : i ? postMessage({ Wc: e, Pc: "checkMailbox" }) : (e = Je[e]) && e.postMessage({ Pc: "checkMailbox" });
  }
  var xr = [];
  function vi(e, t, n, o, u) {
    for (t >>>= 0, u >>>= 0, xr.length = 0, n = u >>> 3, o = u + o >>> 3; n < o; ) {
      var c;
      c = (p(), me)[n++ >>> 0] ? (p(), me)[n++ >>> 0] : (p(), ae)[n++ >>> 0], xr.push(c);
    }
    return (t ? jr[t] : af[e])(...xr);
  }
  var Ei = () => {
    G = 0;
  };
  function Si(e) {
    e >>>= 0, i ? postMessage({ Pc: "cleanupThread", je: e }) : yn(Je[e]);
  }
  function Ai(e) {
  }
  var _t = (e) => {
  ...
  function Ii(e) {
    var t = (...n) => {
      ...
      try {
        ...
        W || (Rt.pop(), Me && Ge === 1 && Rt.length === 0 && (Ge = 0, G += 1, _t(wa), typeof Fibers < "u" && Fibers.De()));
      }
    };
    return jn.set(e, t), t;
  }
  var Ge = 0, Me = null, zn = 0, Rt = [], Lr = /* @__PURE__ */ new Map(), Vn = /* @__PURE__ */ new Map(), jn = /* @__PURE__ */ new Map(), xi = 0, Br = null, Li = [], Hn = (e) => (function(t) {
    if (!W) {
      if (Ge === 0) {
        var n = !1, o = !1;
        t((u = 0) => {
          if (!W && (zn = u, n = !0, o)) {
            Ge = 2, _t(() => ga(Me)), typeof MainLoop < "u" && MainLoop.Yd && MainLoop.resume(), u = !1;
            try {
              var c = (function() {
                var E = (p(), x)[Me + 8 >>> 2 >>> 0];
            ...
            if (!Me) {
              var b = Br;
              b && (Br = null, (u ? b.reject : b.resolve)(c), h = !0);
            }
            if (u && !h) throw c;
          }
        }), o = !0, n || (Ge = 1, Me = (function() {
          var u = pt(65548), c = u + 12;
          if ((p(), A)[u >>> 2 >>> 0] = c, (p(), A)[u + 4 >>> 2 >>> 0] = c + 65536, c = Rt[0], !Lr.has(c)) {
            var h = xi++;
            Lr.set(c, h), Vn.set(h, c);
          }
          return c = Lr.get(c), (p(), x)[u + 8 >>> 2 >>> 0] = c, u;
        })(), typeof MainLoop < "u" && MainLoop.Yd && MainLoop.pause(), _t(() => ba(Me)));
      } else Ge === 2 ? (Ge = 0, _t(Ta), xe(Me), Me = null, Li.forEach(he)) : Te(`invalid state: ${Ge}`);
      return zn;
    }
  })((t) => {
    e().then(t);
  ...
  var Or = [], Oi = (e) => {
    ...
    for (var n = Array(e), o = 0; o < e; ++o) {
      var u = o, c = (p(), A)[t + 4 * o >>> 2 >>> 0], h = Er[c];
      if (h === void 0) throw e = `parameter ${o}`, c = lo(c), t = Oe(c), xe(c), new dt(`${e} has unknown type ${t}`);
      n[u] = h;
    }
    return n;
  }, Ui = (e, t, n) => {
  ...
  function Di(e, t, n) {
    ...
    return b += `(${e})`, o.ge || (h.toReturnWire = t, h.emval_returnValue = Ui, b = `return emval_returnValue(toReturnWire, destructorsRef, ${b})`), b = `return function (handle, methodName, destructorsRef, args) {
  ${b}
  ...
  function qn(e, t, n, o, u) {
    return Or[e >>> 0](t >>> 0, n >>> 0, o >>> 0, u >>> 0);
  }
  function Wi(e, t, n, o, u) {
    return qn(e >>> 0, t >>> 0, n >>> 0, o >>> 0, u >>> 0);
  }
  function Fi() {
    return Ie([]);
  }
  ...
  var Yn = (e) => e % 4 == 0 && (e % 100 != 0 || e % 400 == 0), Jn = [0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335], Xn = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
  function qi(e, t) {
    ...
    var n = (Yn(e.getFullYear()) ? Jn : Xn)[e.getMonth()] + e.getDate() - 1 | 0;
  ...
  function Yi(e) {
    ...
    return 0 > n ? (p(), x)[e + 32 >>> 2 >>> 0] = +(u != c && h == o) : 0 < n != (h == o) && (u = Math.max(c, u), t.setTime(t.getTime() + 6e4 * ((0 < n ? h : u) - o))), (p(), x)[e + 24 >>> 2 >>> 0] = t.getDay(), n = (Yn(t.getFullYear()) ? Jn : Xn)[t.getMonth()] + t.getDate() - 1 | 0, (p(), x)[e + 28 >>> 2 >>> 0] = n, (p(), x)[e >>> 2 >>> 0] = t.getSeconds(), (p(), x)[e + 4 >>> 2 >>> 0] = t.getMinutes(), (p(), x)[e + 8 >>> 2 >>> 0] = t.getHours(), (p(), x)[e + 12 >>> 2 >>> 0] = t.getDate(), (p(), x)[e + 16 >>> 2 >>> 0] = t.getMonth(), (p(), x)[e + 20 >>> 2 >>> 0] = t.getYear(), e = t.getTime(), BigInt(isNaN(e) ? -1 : e / 1e3);
  }
  function Zn(e, t, n, o, u, c, h) {
    return i ? V(16, 1, e, t, n, o, u, c, h) : -52;
  }
  function Kn(e, t, n, o, u, c) {
  ...
  function Qn(e, t) {
    ...
    var n = setTimeout(() => {
      delete Tt[e], he(() => Oo(e, performance.timeOrigin + performance.now()));
    }, t);
  ...
  var Mr = [], eo = (e, t) => {
    ...
    for (var n; n = (p(), J)[e++ >>> 0]; ) {
      var o = n != 105;
      t += (o &= n != 112) && t % 8 ? 4 : 0, Mr.push(n == 112 ? (p(), A)[t >>> 2 >>> 0] : n == 106 ? (p(), me)[t >>> 3 >>> 0] : n == 105 ? (p(), x)[t >>> 2 >>> 0] : (p(), ae)[t >>> 3 >>> 0]), t += o ? 8 : 4;
    }
    return Mr;
  };
  function eu(e, t, n) {
    return e >>>= 0, t = eo(t >>> 0, n >>> 0), jr[e](...t);
  }
  function tu(e, t, n) {
    return e >>>= 0, t = eo(t >>> 0, n >>> 0), jr[e](...t);
  }
  ...
  }, vt = (e) => (p(), A)[e >>> 2 >>> 0] + 4294967296 * (p(), x)[e + 4 >>> 2 >>> 0], de = [], fu = (e, t) => {
    de[e >>> 0] = t;
  }, Re = [], kt = [], lt = (e, t) => {
    kt[e] = new Promise((n) => t.finally(() => n(e)));
  }, L = (e) => {
    if (e) return de[e >>> 0];
  }, cu = (e, t) => {
    ...
    G += 1, lt(t, h.requestDevice(e).then((O) => {
      --G, he(() => {
        de[u >>> 0] = O.queue, de[o >>> 0] = O, G += 1, lt(n, O.lost.then((ue) => {
          ...
          var fe = P();
          ue = Ue(ue.error.message), Io(o, ye, ue), D(fe);
        }, "adapterInfo" in O || (O.adapterInfo = h.info), Wr(t, 1, o, 0);
      });
    }, (O) => {
      --G, he(() => {
        var ue = P(), ye = Ue(O.message);
        Wr(t, 3, o, ye), n && Rr(n, 4, ye), D(ue);
      });
    }));
  }
  function wu(e) {
  ...
  function vu(e, t, n, o, u) {
    ...
    Re[e] = [], u == 4294967295 && (u = void 0), G += 1, lt(t, c.mapAsync(n, o >>> 0, u).then(() => {
      ...
      --G, he(() => {
        P();
        var b = Ue(h.message);
        Nr(t, h.name === "AbortError" ? 4 : h.name === "OperationError" ? 3 : 0, b), delete Re[e];
      });
    }));
  }
  function Eu(e) {
  ...
  function Au(e, t, n) {
    ...
    return de[n >>> 0] = u, o && (Re[n] = []), !0;
  }
  function Iu(e, t, n, o) {
    e >>>= 0, t = ce(t), o >>>= 0, n = du(n >>> 0), e = L(e), G += 1, lt(t, e.createComputePipelineAsync(n).then((u) => {
      --G, he(() => {
        de[o >>> 0] = u, _r(t, 1, o, 0);
      });
    }, (u) => {
      --G, he(() => {
        var c = P(), h = Ue(u.message);
        _r(t, u.reason === "validation" ? 3 : u.reason === "internal" ? 4 : 0, o, h), D(c);
      });
    }));
  }
  function xu(e, t, n) {
  ...
  function Bu(e, t) {
    t = ce(t), e = L(e >>> 0), G += 1, lt(t, e.popErrorScope().then((n) => {
      --G, he(() => {
        ...
        var u = P(), c = n ? Ue(n.message) : 0;
        kr(t, 1, o, c), D(u);
      });
    }, (n) => {
      --G, he(() => {
        var o = P(), u = Ue(n.message);
        kr(t, 1, 5, u), D(o);
      });
    }));
  }
    ...
    "gpu" in navigator ? (G += 1, lt(t, navigator.gpu.requestAdapter(u).then((c) => {
      --G, he(() => {
        if (c) de[o >>> 0] = c, Et(t, 1, o, 0);
          ...
          var h = P(), b = Ue("WebGPU not available on this browser (requestAdapter returned null)");
          Et(t, 3, o, b), D(h);
        }
      });
    }, (c) => {
      --G, he(() => {
        var h = P(), b = Ue(c.message);
        Et(t, 4, o, b), D(h);
      });
    }))) : (u = P(), e = Ue("WebGPU not available on this browser (navigator.gpu is not available)"), Et(t, 3, o, e), D(u));
  }
  function Mu(e, t, n) {
  ...
  var Cr, Dr = {}, oo = () => {
    if (!Cr) {
      ...
      for (e in Dr) Dr[e] === void 0 ? delete t[e] : t[e] = Dr[e];
      var n = [];
  ...
  function so(e, t) {
    if (i) return V(20, 1, e, t);
    e >>>= 0, t >>>= 0;
    var n = oo();
    for (var o of ((p(), A)[e >>> 2 >>> 0] = n.length, e = 0, n)) e += _e(o) + 1;
  ...
  function co(e, t, n, o) {
    ...
    for (var u = 0, c = 0; c < n; c++) {
      ...
      t += 8;
      for (var E = 0; E < b; E++) {
        var I = e, F = (p(), J)[h + E >>> 0], j = Uu[I];
        F === 0 || F === 10 ? ((I === 1 ? q : B)(xn(j)), j.length = 0) : j.push(F);
      }
      u += b;
    }
  ...
  function Ju(e, t) {
    ...
    e >>>= 0, t = { label: Ne((t >>>= 0) + 4), layout: L((p(), A)[t + 12 >>> 2 >>> 0]), entries: (function(u, c) {
  ...
  function Ku(e, t) {
    ...
    return u && Pe(n, u, o), n = u, o = _e(e.vendor), Wt(t + 4, n, o), n += o, o = _e(e.architecture), Wt(t + 12, n, o), n += o, o = _e(e.device), Wt(t + 20, n, o), Wt(t + 28, n + o, _e(e.description)), (p(), x)[t + 36 >>> 2 >>> 0] = 2, e = e.isFallbackAdapter ? 3 : 4, (p(), x)[t + 40 >>> 2 >>> 0] = e, (p(), A)[t + 44 >>> 2 >>> 0] = 0, (p(), A)[t + 48 >>> 2 >>> 0] = 0, 1;
  }
  ...
  function ef(e, t) {
    ...
    var o = 0, u = 0;
    for (let c of n.features) 0 <= (n = Qu[c]) && ((p(), x)[e + o >>> 2 >>> 0] = n, o += 4, u++);
  ...
  i || (function() {
    ...
    Ae.push(async () => {
      var t = (async function() {
        if (!i) return Promise.all(We.map(wn));
      })();
      Be++, await t, --Be == 0 && ee && (t = ee, ee = null, t());
    });
  })(), i || (Fe = new WebAssembly.Memory({ initial: 256, maximum: 65536, shared: !0 }), se()), r.wasmBinary && (g = r.wasmBinary), r.stackSave = () => P(), r.stackRestore = (e) => D(e), r.stackAlloc = (e) => Gt(e), r.setValue = function(e, t, n = "i8") {
  ...
  var lo, po, Pr, Ft, xe, pt, mo, ho, yo, bo, wo, go, To, vo, Eo, So, Ao, _r, Rr, Nr, kr, Et, Wr, Io, Fr, xo, Lo, Bo, Gr, Oo, Mo, $r, N, St, Uo, D, Gt, P, Co, zr, Do, Po, _o, Vr, Ro, No, ko, Wo, Fo, Go, $o, zo, Vo, jo, Ho, qo, Yo, Jo, Xo, Zo, Ko, Qo, ea, ta, ra, na, oa, aa, sa, ia, ua, fa, ca, da, la, pa, ma, ha, ya, ba, wa, ga, Ta, ke, af = [Ye, br, En, Ln, Bn, On, Mn, Un, Cn, Dn, Pn, _n, Rn, Nn, kn, Wn, Zn, Kn, Qn, ao, so, io, uo, fo, co], jr = { 1028716: (e, t, n, o, u) => {
    if (r === void 0 || !r.Vc) return 1;
    if ((e = ct(Number(e >>> 0))).startsWith("./") && (e = e.substring(2)), !(e = r.Vc.get(e))) return 2;
    ...
    r.Td(e, (p(), J).subarray(t >>> 0, t + n >>> 0));
  }, 1029604: () => r.ne(), 1029646: (e) => {
    r.kd(e);
  ...
  function wf(e, t, n, o, u, c, h, b) {
    var E = P();
    try {
      ra(e, t, n, o, u, c, h, b);
    } catch (I) {
      if (D(E), I !== I + 0) throw I;
      N(1, 0);
    }
  }
  function gf(e, t, n, o, u, c, h, b, E, I, F, j) {
    var te = P();
    try {
      Yo(e, t, n, o, u, c, h, b, E, I, F, j);
    } catch (O) {
      if (D(te), O !== O + 0) throw O;
      N(1, 0);
    }
  }
  ...
  function Cf(e, t, n, o, u, c) {
    var h = P();
    try {
      return fa(e, t, n, o, u, c);
    } catch (b) {
      if (D(h), b !== b + 0) throw b;
      N(1, 0);
    }
  }
  function Df(e, t, n, o, u, c, h, b) {
    var E = P();
    try {
  ...
  function Nf(e, t, n, o, u, c, h, b, E, I, F, j) {
    var te = P();
    try {
      return pa(e, t, n, o, u, c, h, b, E, I, F, j);
    } catch (O) {
      if (D(te), O !== O + 0) throw O;
      N(1, 0);
    }
  }
  function kf(e, t, n, o, u, c, h, b, E, I, F) {
    var j = P();
    try {
      ma(e, t, n, o, u, c, h, b, E, I, F);
    } catch (te) {
      if (D(j), te !== te + 0) throw te;
      N(1, 0);
    }
  }
  function Wf(e, t, n, o, u, c, h, b, E, I, F, j, te, O, ue, ye) {
    var fe = P();
    try {
      ha(e, t, n, o, u, c, h, b, E, I, F, j, te, O, ue, ye);
    } catch (Ze) {
      if (D(fe), Ze !== Ze + 0) throw Ze;
      N(1, 0);
    }
  }
  function Ff(e, t, n) {
  ...
  function $t() {
    if (0 < Be) ee = $t;
      ...
      0 < Be ? ee = $t : (r.calledRun = !0, W || (yr(), U?.(r)));
    }
  }
  return i || (ke = await bt(), $t()), r.PTR_SIZE = 4, r.webgpuInit = (e) => {
    ...
    r.webgpuRegisterBuffer = (b, E, I) => {
      ...
      return async () => {
        ...
        try {
          let O = F.createCommandEncoder();
          return O.copyBufferToBuffer(b, 0, te, 0, j), F.queue.submit([O.finish()]), await te.mapAsync(GPUMapMode.READ), te.getMappedRange().slice(0, E);
        } finally {
          te.destroy();
        }
      };
      ...
      let O = (te = n.createBuffer({ mappedAtCreation: !0, size: E, usage: 6 })).getMappedRange();
      new Uint8Array(O).set(new Uint8Array(I, F, j)), te.unmap(), (I = n.createCommandEncoder()).copyBufferToBuffer(te, 0, b, 0, E), n.queue.submit([I.finish()]), te.destroy();
    };
  }, r.webnnInit = (e) => {
    let t = e[0];
    [r.ne, r.kd, r.webnnEnsureTensor, r.Td, r.webnnDownloadTensor, r.me, r.webnnEnableTraceEvent] = e.slice(1), r.webnnReleaseTensorId = r.kd, r.webnnUploadTensor = r.Td, r.webnnRegisterMLContext = r.me, r.webnnOnRunStart = (n) => t.onRunStart(n), r.webnnOnRunEnd = t.onRunEnd.bind(t), r.webnnOnReleaseSession = (n) => {
      t.onReleaseSession(n);
    }, r.webnnCreateMLTensorDownloader = (n, o) => t.createMLTensorDownloader(n, o), r.webnnRegisterMLTensor = (n, o, u, c) => t.registerMLTensor(n, o, u, c), r.webnnCreateMLContext = (n) => t.createMLContext(n), r.webnnRegisterMLConstant = (n, o, u, c, h, b) => t.registerMLConstant(n, o, u, c, h, r.Vc, b), r.webnnRegisterGraphInput = t.registerGraphInput.bind(t), r.webnnIsGraphInput = t.isGraphInput.bind(t), r.webnnRegisterGraphOutput = t.registerGraphOutput.bind(t), r.webnnIsGraphOutput = t.isGraphOutput.bind(t), r.webnnCreateTemporaryTensor = t.createTemporaryTensor.bind(t), r.webnnIsGraphInputOutputTypeSupported = t.isGraphInputOutputTypeSupported.bind(t);
...
var Kf, Qf, ns = k(() => {
  "use strict";
  Kf = ts, Qf = globalThis.self?.name?.startsWith("em-pthread"), Qf && ts();
}), ss, rn, ec, ge, is, tn, tc, rc, us, nc, os, fs, as, cs, Yt = k(() => {
  "use strict";
  qt(), ss = typeof location > "u" ? void 0 : location.origin, rn = import.meta.url > "file:" && import.meta.url < "file;", ec = () => {
    if (rn) {
      let a = URL;
  ...
  }, rc = (a, r) => `${r ?? "./"}${a}`, us = async (a) => {
    let s = await (await fetch(a, { credentials: "same-origin" })).blob();
    return URL.createObjectURL(s);
  }, nc = async (a) => (await import(
    /*webpackIgnore:true*/
    /*@vite-ignore*/
    a
  )).default, os = (es(), zt(Qa)).default, fs = async () => {
    if (!ge) throw new Error("Failed to load proxy worker: cannot determine the script source URL.");
    if (tn(ge)) return [void 0, os()];
    let a = await us(ge);
    return [a, os(a)];
  }, as = (ns(), zt(rs)).default, cs = async (a, r, s, f) => {
    let i = as && !(a || r);
    if (i) if (ge) i = tn(ge) || f && !s;
    else if (f && !s) i = !0;
    else throw new Error("cannot determine the script source URL.");
    if (i) return [void 0, as];
    {
      let d = "ort-wasm-simd-threaded.asyncify.mjs", l = a ?? tc(d, r), m = s && l && !tn(l, r), y = m ? await us(l) : l ?? rc(d, r);
      return [m ? y : void 0, await nc(y)];
    }
  };
}), nn, on, nr, ds, oc, ac, sc, Jt, z, je = k(() => {
  "use strict";
  Yt(), on = !1, nr = !1, ds = !1, oc = () => {
    ...
    if (nr) throw new Error("multiple calls to 'initializeWebAssembly()' detected.");
    if (ds) throw new Error("previous call to 'initializeWebAssembly()' failed.");
    nr = !0;
    let r = a.initTimeout, s = a.numThreads;
    if (a.simd !== !1) {
      if (a.simd === "relaxed") {
        if (!sc()) throw new Error("Relaxed WebAssembly SIMD is not supported in the current environment.");
    ...
    let i = a.wasmPaths, d = typeof i == "string" ? i : void 0, l = i?.mjs, m = l?.href ?? l, y = i?.wasm, w = y?.href ?? y, T = a.wasmBinary, [g, v] = await cs(m, d, s > 1, !!T || !!w), S = !1, U = [];
    if (r > 0 && U.push(new Promise((R) => {
      ...
      let C = { numThreads: s };
      if (T) C.wasmBinary = T, C.locateFile = (M) => M;
      else if (w || d) C.locateFile = (M) => w ?? d + M;
      else if (m && m.indexOf("blob:") !== 0) C.locateFile = (M) => new URL(M, m).href;
      else if (g) {
        let M = is();
        M && (C.locateFile = (q) => M + q);
      }
      v(C).then((M) => {
        nr = !1, on = !0, nn = M, R(), g && URL.revokeObjectURL(g);
      }, (M) => {
        nr = !1, ds = !0, H(M);
      });
  ...
  je(), or(), ls = (a) => {
    ...
    try {
      ...
      a?.terminate === void 0 && (i.terminate = !1);
      let d = 0;
  ...
  je(), or(), ic = (a) => {
    ...
    r.use_ort_model_bytes_directly || (r.use_ort_model_bytes_directly = "1"), a.executionProviders && a.executionProviders.some((s) => (typeof s == "string" ? s : s.name) === "webgpu") && (a.enableMemPattern = !1);
  }, Bt = (a, r, s, f) => {
    ...
    for (let i of f) {
      ...
      switch (d) {
        case "webnn":
          if (d = "WEBNN", Bt(a, "session.disable_quant_qdq", "1", s), Bt(a, "session.disable_qdq_constant_folding", "1", s), typeof i != "string") {
            let v = i?.deviceType;
            v && Bt(a, "deviceType", v, s);
          }
          break;
        case "webgpu":
          {
            ...
            if (typeof i != "string") {
              ...
              if (typeof U == "boolean" && U && ot(l, "enableGraphCapture", "1", s), typeof S.preferredLayout == "string" && ot(l, "preferredLayout", S.preferredLayout, s), S.forceCpuNodeNames) {
...
`), s);
            ...
            if (v) {
              let [S, U, R] = v;
              ot(l, "deviceId", S.toString(), s), ot(l, "webgpuInstance", U.toString(), s), ot(l, "webgpuDevice", R.toString(), s);
            }
          }
          break;
        case "wasm":
    ...
    fc(i);
    try {
      let d = ic(i.graphOptimizationLevel ?? "all"), l = uc(i.executionMode ?? "sequential"), m = typeof i.logId == "string" ? be(i.logId, f) : 0, y = i.logSeverityLevel ?? 2;
      ...
      if (s = r._OrtCreateSessionOptions(d, !!i.enableCpuMemArena, !!i.enableMemPattern, l, !!i.enableProfiling, 0, m, y, w, T), s === 0 && $("Can't create session options."), i.executionProviders && await cc(s, i, f), i.enableGraphCapture !== void 0) {
        if (typeof i.enableGraphCapture != "boolean") throw new Error(`enableGraphCapture must be a boolean value: ${i.enableGraphCapture}`);
        Bt(s, "enableGraphCapture", i.enableGraphCapture.toString(), f);
      }
  ...
  }, sr = (a) => a === "float32" || a === "float16" || a === "int32" || a === "int64" || a === "uint32" || a === "uint8" || a === "bool" || a === "uint4" || a === "int4", ir = (a) => a === "float32" || a === "float16" || a === "int32" || a === "int64" || a === "uint32" || a === "uint64" || a === "int8" || a === "uint8" || a === "bool" || a === "uint4" || a === "int4", an = (a) => {
    switch (a) {
      case "none":
        return 0;
      case "cpu":
  ...
  "use strict";
  st(), ys = (a, r) => new (at(r))(a);
}), dc, lc, ws, gs, Ts, pc, pe, un = k(() => {
  "use strict";
  st(), dc = ["V", "I", "W", "E", "F"], lc = (a, r) => {
    console.log(`[${dc[a]},${(/* @__PURE__ */ new Date()).toISOString()}]${r}`);
  }, Ts = (a, r) => {
    ws = a, gs = r;
  }, pc = (a, r) => {
    let s = Ot(a), f = Ot(ws);
...
}), Es, cn, Ss, mc, vs, hc, As, ur, fr, fn, Is, xs = k(() => {
  "use strict";
  st(), un(), Es = /* @__PURE__ */ new Map([["float32", 32], ["float16", 16], ["int32", 32], ["uint32", 32], ["int64", 64], ["uint64", 64], ["int8", 8], ["uint8", 8], ["int4", 4], ["uint4", 4]]), cn = (a, r) => {
    if (r === "int32") return a;
    let s = Es.get(r);
  ...
  }, mc = 1, vs = () => mc++, hc = /* @__PURE__ */ new Map([["int8", "int32"], ["uint8", "int32"], ["uint32", "int32"], ["int64", "int32"]]), As = (a, r) => {
    let s = Es.get(a);
    ...
    constructor(r, s) {
      this.tensorManager = r, this.wrapper = s;
    }
    get tensorWrapper() {
      return this.wrapper;
    }
    ...
    async ensureTensor(r, s, f, i) {
      ...
      if (!l?.input.dataTypes.includes(s)) {
        if (m = hc.get(s), !m || l?.input.dataTypes.includes(m)) throw new Error(`WebNN backend does not support data type: ${s}`);
      ...
      return this.wrapper = await this.tensorManager.getCachedTensor(r, s, f, y, !0, !0, m), i && this.activeUpload && (this.wrapper.write(this.activeUpload), this.activeUpload = void 0), this.wrapper.tensor;
    }
    upload(r) {
      let s = r;
      if (this.wrapper) {
        if (this.wrapper.fallbackType) if (this.wrapper.fallbackType === "int32") s = cn(r, this.wrapper.type), this.wrapper.setIsDataConverted(!0);
    ...
    constructor(r) {
      this.backend = r, this.tensorTrackersById = /* @__PURE__ */ new Map(), this.freeTensors = [], this.externalTensors = /* @__PURE__ */ new Set();
    }
    getMLContext(r) {
    ...
    reserveTensorId() {
      let r = vs();
      return this.tensorTrackersById.set(r, new fr(this)), r;
    }
    releaseTensorId(r) {
    ...
    registerTensor(r, s, f, i) {
      let d = this.getMLContext(r), l = vs(), m = new ur({ sessionId: r, context: d, tensor: s, dataType: f, shape: i });
      return this.tensorTrackersById.set(l, new fr(this, m)), this.externalTensors.add(m), l;
    }
    async getCachedTensor(r, s, f, i, d, l, m) {
      ...
      return new ur({ sessionId: r, context: y, tensor: w, dataType: s, shape: f, fallbackDataType: m });
    }
    releaseTensor(r) {
...
var cr, yc, dn, Bs = k(() => {
  "use strict";
  st(), je(), bs(), xs(), un(), cr = /* @__PURE__ */ new Map([[1, "float32"], [10, "float16"], [6, "int32"], [12, "uint32"], [7, "int64"], [13, "uint64"], [22, "int4"], [21, "uint4"], [3, "int8"], [2, "uint8"], [9, "uint8"]]), yc = (a, r) => {
    if (a === r) return !0;
    if (a === void 0 || r === void 0) return !1;
    let s = Object.keys(a).sort(), f = Object.keys(r).sort();
    return s.length === f.length && s.every((i, d) => i === f[d] && a[i] === r[i]);
  }, dn = class {
    constructor(r) {
      this.tensorManager = Is(this), this.mlContextBySessionId = /* @__PURE__ */ new Map(), this.sessionIdsByMLContext = /* @__PURE__ */ new Map(), this.mlContextCache = [], this.sessionGraphInputs = /* @__PURE__ */ new Map(), this.sessionGraphOutputs = /* @__PURE__ */ new Map(), this.temporaryGraphInputs = [], this.temporaryGraphOutputs = [], this.temporarySessionTensorIds = /* @__PURE__ */ new Map(), this.mlOpSupportLimitsBySessionId = /* @__PURE__ */ new Map(), Ts(r.logLevel, !!r.debug);
    }
    ...
    async createMLContext(r) {
      ...
      let s = this.mlContextCache.findIndex((f) => yc(f.options, r));
    ...
    async createTemporaryTensor(r, s, f) {
      ...
      await this.tensorManager.ensureTensor(r, d, i, f, !1);
    ...
    registerMLConstant(r, s, f, i, d, l, m = !1) {
      ...
      switch (d.dataType) {
        case "float32":
          ...
          break;
        case "float16":
          g = typeof Float16Array < "u" ? new Float16Array(T) : new Uint16Array(T);
          break;
        case "int32":
          g = new Int32Array(T);
          break;
    ...
    isGraphInput(r, s) {
      ...
      return f ? f.includes(s) : !1;
    }
    isGraphOutput(r, s) {
      let f = this.sessionGraphOutputs.get(r);
      return f ? f.includes(s) : !1;
    }
    isGraphInputOutputTypeSupported(r, s, f = !0) {
      let i = cr.get(He(s)), d = this.mlOpSupportLimitsBySessionId.get(r);
      return typeof i > "u" ? !1 : f ? !!d?.input.dataTypes.includes(i) : !!d?.output.dataTypes.includes(i);
    }
    flush() {
    }
  };
}), bc, Xt, Zt, it, wc, Os, xt, Kt, Qt, Ms, er, tr, rr, en = k(() => {
  "use strict";
  Ve(), ps(), hs(), st(), je(), or(), sn(), bc = (a, r) => {
    z()._OrtInit(a, r) !== 0 && $("Can't initialize onnxruntime.");
  }, Xt = async (a) => {
    bc(a.wasm.numThreads, Ot(a.logLevel));
  }, Zt = async (a, r) => {
    z().asyncInit?.();
    ...
    if (r === "webgpu" && z().webgpuInit((f) => {
      ...
      let f = new (Bs(), zt(Ls)).WebNNBackend(a);
    ...
    try {
      if ([l, y] = await ms(r), r?.externalData && i.mountExternalData) {
        ...
        for (let W of r.externalData) {
          ...
          B.push(Mt(typeof W == "string" ? W : W.data).then((p) => {
      ...
      for (let B of r?.executionProviders ?? []) if ((typeof B == "string" ? B : B.name) === "webnn") {
        if (i.shouldTransferToMLTensor = !1, typeof B != "string") {
          let oe = B, p = oe?.context, ne = oe?.gpuDevice, X = oe?.deviceType, J = oe?.powerPreference;
      ...
      d = await i._OrtCreateSession(s, f, l), i.webgpuOnCreateSession?.(d), d === 0 && $("Can't create a session."), i.jsepOnCreateSession?.(), i.currentContext && (i.webnnRegisterMLContext(d, i.currentContext), i.currentContext = void 0, i.shouldTransferToMLTensor = !0);
      let [g, v] = wc(d), S = !!r?.enableGraphCapture, U = [], R = [], H = [], C = [], M = [];
      for (let B = 0; B < g; B++) {
        let [W, oe, p] = Os(d, B);
        W === 0 && $("Can't get an input name."), w.push(W);
        let ne = i.UTF8ToString(W);
        U.push(ne), H.push(oe === 0 ? { name: ne, isTensor: !1 } : { name: ne, isTensor: !0, type: ar(oe), shape: p });
      }
      for (let B = 0; B < v; B++) {
        let [W, oe, p] = Os(d, B + g);
        W === 0 && $("Can't get an output name."), T.push(W);
        let ne = i.UTF8ToString(W);
        R.push(ne), C.push(oe === 0 ? { name: ne, isTensor: !1 } : { name: ne, isTensor: !0, type: ar(oe), shape: p });
        {
          ...
          if (X !== "cpu" && X !== "cpu-pinned" && X !== "gpu-buffer" && X !== "ml-tensor") throw new Error(`Not supported preferred output location: ${X}.`);
          if (S && X !== "gpu-buffer") throw new Error(`Not supported preferred output location: ${X}. Only 'gpu-buffer' location is supported when enableGraphCapture is true.`);
          M.push(X);
        }
      }
      let q = null;
      return M.some((B) => B === "gpu-buffer" || B === "ml-tensor" || B === "ml-tensor-cpu-output") && (m = i._OrtCreateBinding(d), m === 0 && $("Can't create IO binding."), q = { handle: m, outputPreferredLocations: M, outputPreferredLocationsEncoded: M.map((B) => B === "ml-tensor-cpu-output" ? "ml-tensor" : B).map((B) => an(B)) }), it.set(d, [d, w, T, q, S, !1]), [d, U, R, H, C];
    } catch (g) {
  ...
  }, Ms = async (a, r, s, f, i, d, l = !1) => {
    if (!a) {
      r.push(0);
      return;
    }
    let m = z(), y = m.PTR_SIZE, w = a[0], T = a[1], g = a[3], v = g, S, U;
    ...
    if (g === "gpu-buffer") {
      ...
      let C = a[2];
      if (Array.isArray(C)) {
        U = y * C.length, S = m._malloc(U), s.push(S);
        for (let M = 0; M < C.length; M++) {
          if (typeof C[M] != "string") throw new TypeError(`tensor data at index ${M} is not a string`);
          m.setValue(S + M * y, be(C[M], s), "*");
        }
      } else {
        let M = m.webnnIsGraphInput, q = m.webnnIsGraphOutput;
        if (w !== "string" && M && q) {
          let B = m.UTF8ToString(i);
          if (M(f, B) || q(f, B)) {
            let W = He(w);
            U = mt(W, T), v = "ml-tensor";
            let oe = m.webnnCreateTemporaryTensor, p = m.webnnUploadTensor;
            if (!oe || !p) throw new Error('Tensor location "ml-tensor" is not supported without using WebNN.');
            let ne = await oe(f, W, T);
            p(ne, new Uint8Array(C.buffer, C.byteOffset, C.byteLength)), S = ne;
          } else U = C.byteLength, S = m._malloc(U), s.push(S), m.HEAPU8.set(new Uint8Array(C.buffer, C.byteOffset, U), S);
        } else U = C.byteLength, S = m._malloc(U), s.push(S), m.HEAPU8.set(new Uint8Array(C.buffer, C.byteOffset, U), S);
      }
    }
    let R = m.stackSave(), H = m.stackAlloc(4 * T.length);
    try {
      T.forEach((M, q) => m.setValue(H + q * y, M, y === 4 ? "i32" : "i64"));
      let C = m._OrtCreateTensor(He(w), S, U, H, T.length, an(v));
      C === 0 && $(`Can't create tensor for input/output. session=${f}, index=${d}.`), r.push(C);
    } finally {
      m.stackRestore(R);
    }
  }, er = async (a, r, s, f, i, d) => {
    let l = z(), m = l.PTR_SIZE, y = it.get(a);
    if (!y) throw new Error(`cannot run inference. invalid session id: ${a}`);
    let w = y[0], T = y[1], g = y[2], v = y[3], S = y[4], U = y[5], R = r.length, H = f.length, C = 0, M = [], q = [], B = [], W = [], oe = [], p = l.stackSave(), ne = l.stackAlloc(R * m), X = l.stackAlloc(R * m), J = l.stackAlloc(H * m), Ce = l.stackAlloc(H * m);
    try {
      [C, M] = ls(d), $e("wasm prepareInputOutputTensor");
      for (let _ = 0; _ < R; _++) await Ms(s[_], q, W, a, T[r[_]], r[_], S);
      for (let _ = 0; _ < H; _++) await Ms(i[_], B, W, a, g[f[_]], R + f[_], S);
      ze("wasm prepareInputOutputTensor");
      for (let _ = 0; _ < R; _++) l.setValue(ne + _ * m, q[_], "*"), l.setValue(X + _ * m, T[r[_]], "*");
      for (let _ = 0; _ < H; _++) l.setValue(J + _ * m, B[_], "*"), l.setValue(Ce + _ * m, g[f[_]], "*");
      if (v && !U) {
        ...
        for (let Y = 0; Y < R; Y++) {
          let we = r[Y];
          await l._OrtBindInput(_, T[we], q[Y]) !== 0 && $(`Can't bind input[${Y}] for session=${a}.`);
        }
        for (let Y = 0; Y < H; Y++) {
          let we = f[Y];
          i[Y]?.[3] ? (oe.push(B[Y]), l._OrtBindOutput(_, g[we], B[Y], 0) !== 0 && $(`Can't bind pre-allocated output[${Y}] for session=${a}.`)) : l._OrtBindOutput(_, g[we], 0, me[we]) !== 0 && $(`Can't bind output[${Y}] to ${ae[Y]} for session=${a}.`);
        }
        ze("wasm bindInputsOutputs"), it.set(a, [w, T, g, v, S, !0]);
      }
      l.jsepOnRunStart?.(w), l.webnnOnRunStart?.(w);
      let Z;
      v ? Z = await l._OrtRunWithBinding(w, v.handle, H, J, C) : Z = await l._OrtRun(w, X, ne, R, Ce, H, J, C), Z !== 0 && $("failed to call OrtRun().");
      let x = [], A = [];
      $e("wasm ProcessOutputTensor");
      for (let _ = 0; _ < H; _++) {
        let ae = Number(l.getValue(J + _ * m, "*"));
        if (ae === B[_] || oe.includes(B[_])) {
          x.push(i[_]), ae !== B[_] && l._OrtReleaseTensor(ae) !== 0 && $("Can't release tensor.");
          continue;
        }
        let me = l.stackSave(), Y = l.stackAlloc(4 * m), we = !1, re, se = 0;
        try {
          l._OrtGetTensorData(ae, Y, Y + m, Y + 2 * m, Y + 3 * m) !== 0 && $(`Can't access output tensor data on index ${_}.`);
          let Te = m === 4 ? "i32" : "i64", qe = Number(l.getValue(Y, Te));
          se = l.getValue(Y + m, "*");
          let bt = l.getValue(Y + m * 2, "*"), wt = Number(l.getValue(Y + m * 3, Te)), Se = [];
          for (let ee = 0; ee < wt; ee++) Se.push(Number(l.getValue(bt + ee * m, Te)));
          l._OrtFree(bt) !== 0 && $("Can't free memory for tensor dims.");
          let Ae = Se.reduce((ee, K) => ee * K, 1);
          re = ar(qe);
          let Be = v?.outputPreferredLocations[f[_]];
          if (re === "string") {
            if (Be === "gpu-buffer" || Be === "ml-tensor") throw new Error("String tensor is not supported on GPU.");
            let ee = [];
            for (let K = 0; K < Ae; K++) {
              let G = l.getValue(se + K * m, "*"), V = l.getValue(se + (K + 1) * m, "*"), Ye = K === Ae - 1 ? void 0 : V - G;
              ee.push(l.UTF8ToString(G, Ye));
            }
            x.push([re, Se, ee, "cpu"]);
          } else if (Be === "gpu-buffer" && Ae > 0) {
            let ee = l.webgpuGetBuffer;
            if (!ee) throw new Error('preferredLocation "gpu-buffer" is not supported without using WebGPU.');
            let K = ee(se), G = mt(qe, Ae);
            if (G === void 0 || !sr(re)) throw new Error(`Unsupported data type: ${re}`);
            we = !0;
            {
              l.webgpuRegisterBuffer(K, a, se);
              let V = l.webgpuCreateDownloader(K, G, a);
              x.push([re, Se, { gpuBuffer: K, download: async () => {
                let Ye = await V();
                return new (at(re))(Ye);
              }, dispose: () => {
          ...
          } else if (Be === "ml-tensor" && Ae > 0) {
            let ee = l.webnnEnsureTensor, K = l.webnnIsGraphInputOutputTypeSupported;
            if (!ee || !K) throw new Error('preferredLocation "ml-tensor" is not supported without using WebNN.');
            if (mt(qe, Ae) === void 0 || !ir(re)) throw new Error(`Unsupported data type: ${re}`);
            if (!K(a, re, !1)) throw new Error(`preferredLocation "ml-tensor" for ${re} output is not supported by current WebNN Context.`);
            let V = await ee(a, se, qe, Se, !1);
            we = !0, x.push([re, Se, { mlTensor: V, download: l.webnnCreateMLTensorDownloader(se, re), dispose: () => {
              l.webnnReleaseTensorId(se), l._OrtReleaseTensor(ae);
            } }, "ml-tensor"]);
          } else if (Be === "ml-tensor-cpu-output" && Ae > 0) {
            let ee = l.webnnCreateMLTensorDownloader(se, re)(), K = x.length;
            we = !0, A.push((async () => {
              let G = [K, await ee];
              return l.webnnReleaseTensorId(se), l._OrtReleaseTensor(ae), G;
            })()), x.push([re, Se, [], "cpu"]);
          } else {
            let ee = at(re), K = new ee(Ae);
            new Uint8Array(K.buffer, K.byteOffset, K.byteLength).set(l.HEAPU8.subarray(se, se + K.byteLength)), x.push([re, Se, K, "cpu"]);
          }
        } finally {
          l.stackRestore(me), re === "string" && se && l._free(se), we || l._OrtReleaseTensor(ae);
        }
      }
      v && !S && (l._OrtClearBoundOutputs(v.handle) !== 0 && $("Can't clear bound outputs."), it.set(a, [w, T, g, v, S, !1]));
      for (let [_, ae] of await Promise.all(A)) x[_][2] = ae;
      return ze("wasm ProcessOutputTensor"), x;
    } finally {
      l.webnnOnRunEnd?.(w), l.stackRestore(p), s.forEach((Z) => {
        Z && Z[3] === "gpu-buffer" && l.webgpuUnregisterBuffer(Z[2].gpuBuffer);
      }), i.forEach((Z) => {
        Z && Z[3] === "gpu-buffer" && l.webgpuUnregisterBuffer(Z[2].gpuBuffer);
      }), q.forEach((Z) => l._OrtReleaseTensor(Z)), B.forEach((Z) => l._OrtReleaseTensor(Z)), W.forEach((Z) => l._free(Z)), C !== 0 && l._OrtReleaseRunOptions(C), M.forEach((Z) => l._free(Z));
    }
  }, tr = (a) => {
    let r = z(), s = it.get(a);
...
}), ut, Ee, Ut, lr, pr, dr, ln, pn, ht, yt, Tc, Us, Cs, Ds, Ps, _s, Rs, Ns, mn = k(() => {
  "use strict";
  Ve(), en(), je(), Yt(), ut = () => !!Q.wasm.proxy && typeof document < "u", Ut = !1, lr = !1, pr = !1, pn = /* @__PURE__ */ new Map(), ht = (a, r) => {
    let s = pn.get(a);
    s ? s.push(r) : pn.set(a, [r]);
  }, yt = () => {
    if (Ut || !lr || pr || !Ee) throw new Error("worker not ready");
  }, Tc = (a) => {
    switch (a.data.type) {
      case "init-wasm":
        Ut = !1, a.data.err ? (pr = !0, ln[1](a.data.err)) : (lr = !0, ln[0]()), dr && (URL.revokeObjectURL(dr), dr = void 0);
        break;
      case "init-ep":
      case "copy-from":
      case "create":
    ...
    if (!lr) {
      if (Ut) throw new Error("multiple calls to 'initWasm()' detected.");
      if (pr) throw new Error("previous call to 'initWasm()' failed.");
      if (Ut = !0, ut()) return new Promise((a, r) => {
        Ee?.terminate(), fs().then(([s, f]) => {
          try {
            Ee = f, Ee.onerror = (d) => r(d), Ee.onmessage = Tc, ln = [a, r];
            let i = { type: "init-wasm", in: Q };
            !i.in.wasm.wasmPaths && (s || rn) && (i.in.wasm.wasmPaths = { wasm: new URL("ort-wasm-simd-threaded.asyncify.wasm", import.meta.url).href }), Ee.postMessage(i), dr = s;
          } catch (i) {
            r(i);
          }
        }, r);
      ...
      try {
        await Jt(Q.wasm), await Xt(Q), lr = !0;
      } catch (a) {
        throw pr = !0, a;
      } finally {
        Ut = !1;
    ...
    if (ut()) return yt(), new Promise((r, s) => {
      ht("init-ep", [r, s]);
      let f = { type: "init-ep", in: { epName: a, env: Q } };
      Ee.postMessage(f);
    });
    await Zt(Q, a);
    ...
    if (ut()) {
      ...
      return yt(), new Promise((l, m) => {
        ...
        Ee.postMessage(w, rr(y));
      });
    } else return er(a, r, s, f, i, d);
  }, Ns = async (a) => {
    if (ut()) return yt(), new Promise((r, s) => {
  ...
  Ve(), mn(), st(), qt(), sn(), ks = (a, r) => {
    ...
    switch (a[3]) {
      ...
      case "gpu-buffer": {
        let r = a[0];
        if (!sr(r)) throw new Error(`not supported data type: ${r} for deserializing GPU tensor`);
        let { gpuBuffer: s, download: f, dispose: i } = a[2];
        return Le.fromGpuBuffer(s, { dataType: r, dims: a[1], download: f, dispose: i });
      }
      case "ml-tensor": {
        let r = a[0];
        if (!ir(r)) throw new Error(`not supported data type: ${r} for deserializing MLTensor tensor`);
    ...
    async run(r, s, f) {
      ...
      for (let v = 0; v < T.length; v++) g[this.outputNames[m[v]]] = l[v] ?? vc(T[v]);
      return rt(), g;
    }
    startProfiling() {
    }
...
At(Gs, { OnnxruntimeWebAssemblyBackend: () => hr, initializeFlags: () => Fs, wasmBackend: () => Ec });
var Fs, hr, Ec, $s = k(() => {
  "use strict";
  Ve(), mn(), Ws(), Fs = () => {
    (typeof Q.wasm.initTimeout != "number" || Q.wasm.initTimeout < 0) && (Q.wasm.initTimeout = 0);
    let a = Q.wasm.simd;
    if (typeof a != "boolean" && a !== void 0 && a !== "fixed" && a !== "relaxed" && (console.warn(`Property "env.wasm.simd" is set to unknown value "${a}". Reset it to \`false\` and ignore SIMD feature checking.`), Q.wasm.simd = !1), typeof Q.wasm.proxy != "boolean" && (Q.wasm.proxy = !1), typeof Q.wasm.trace != "boolean" && (Q.wasm.trace = !1), typeof Q.wasm.numThreads != "number" || !Number.isInteger(Q.wasm.numThreads) || Q.wasm.numThreads <= 0) if (typeof self < "u" && !self.crossOriginIsolated) Q.wasm.numThreads = 1;
    else {
      let r = typeof navigator > "u" ? qr("node:os").cpus().length : navigator.hardwareConcurrency;
      Q.wasm.numThreads = Math.min(4, Math.ceil((r || 1) / 2));
    }
  }, hr = class {
    async init(r) {
      Fs(), await Us(), await Cs(r);
    }
...
var Xa = "1.27.0";
{
  let a = ($s(), zt(Gs)).wasmBackend;
  Qe("webgpu", a, 5), Qe("webnn", a, 5), Qe("cpu", a, 10), Qe("wasm", a, 10);
}
Object.defineProperty(Q.versions, "web", { value: Xa, enumerable: !0 });

// src/offscreen/preprocess.js
var IMAGENET_MEAN = Object.freeze([0.485, 0.456, 0.406]), IMAGENET_STD = Object.freeze([0.229, 0.224, 0.225]), DEFAULT_MAX_AXIS_SAMPLES = 3, TILE_AGGREGATION = Object.freeze({
  MEAN: "mean-probability",
  MAX: "max-probability",
  TOP2: "top2-mean-probability",
  TOP3: "top3-mean-probability"
}), DEFAULT_TILE_AGGREGATION = TILE_AGGREGATION.MAX;
function planOfficialCenterCrop(width, height, options = {}) {
  let shortEdge = options.shortEdge ?? 440, cropSize = options.cropSize ?? 384;
  for (let [name, value] of Object.entries({ width, height, shortEdge, cropSize }))
    if (!Number.isInteger(value) || value <= 0)
      throw new RangeError(`${name} must be a positive integer`);
  if (cropSize > shortEdge) throw new RangeError("cropSize cannot exceed resized short edge");
  let resizedWidth, resizedHeight;
  return width <= height ? (resizedWidth = shortEdge, resizedHeight = Math.max(shortEdge, Math.floor(height * shortEdge / width))) : (resizedHeight = shortEdge, resizedWidth = Math.max(shortEdge, Math.floor(width * shortEdge / height))), {
    resizedWidth,
    resizedHeight,
...
function axisOrigins(length, tileSize = 384, maxSamples = DEFAULT_MAX_AXIS_SAMPLES) {
  ...
  let last = length - tileSize, count = Math.min(maxSamples, Math.ceil(length / tileSize));
  return count === 1 ? [Math.round(last / 2)] : Array.from({ length: count }, (_, i) => Math.round(last * i / (count - 1)));
}
function planNativeTiles(width, height, options = {}) {
  let tileSize = options.tileSize ?? 384, maxAxisSamples = options.maxAxisSamples ?? DEFAULT_MAX_AXIS_SAMPLES, xs2 = axisOrigins(width, tileSize, maxAxisSamples);
  return axisOrigins(height, tileSize, maxAxisSamples).flatMap((y) => xs2.map((x) => ({ x, y, size: tileSize })));
}
function rgbaToImageNetNchw(rgba, width, height) {
  let pixels = width * height;
  if (rgba.length !== pixels * 4)
    throw new RangeError(`expected ${pixels * 4} RGBA bytes, got ${rgba.length}`);
  let output = new Float32Array(pixels * 3);
  for (let i = 0; i < pixels; i += 1) {
    let source = i * 4;
    output[i] = ((rgba[source] ?? 0) / 255 - IMAGENET_MEAN[0]) / IMAGENET_STD[0], output[pixels + i] = ((rgba[source + 1] ?? 0) / 255 - IMAGENET_MEAN[1]) / IMAGENET_STD[1], output[pixels * 2 + i] = ((rgba[source + 2] ?? 0) / 255 - IMAGENET_MEAN[2]) / IMAGENET_STD[2];
  }
  return output;
}
var workCanvas = null, workCtx = null;
function getWorkContext(width, height) {
  if ((!workCanvas || !workCtx || workCanvas.width !== width || workCanvas.height !== height) && (workCanvas = new OffscreenCanvas(width, height), workCtx = workCanvas.getContext("2d", { alpha: !1, willReadFrequently: !0 })), !workCtx) throw new Error("2D canvas unavailable for model preprocessing");
  return workCtx;
}
function bitmapTileToNchw(bitmap, tile) {
  let ctx = getWorkContext(tile.size, tile.size);
  ctx.fillStyle = "rgb(124, 116, 104)", ctx.fillRect(0, 0, tile.size, tile.size);
  let sx = Math.max(0, tile.x), sy = Math.max(0, tile.y), dx = sx - tile.x, dy = sy - tile.y, copyWidth = Math.min(tile.size - dx, bitmap.width - sx), copyHeight = Math.min(tile.size - dy, bitmap.height - sy);
  ...
  let rgba = ctx.getImageData(0, 0, tile.size, tile.size).data;
  return rgbaToImageNetNchw(rgba, tile.size, tile.size);
}
function bitmapOfficialCenterToNchw(bitmap) {
  let plan = planOfficialCenterCrop(bitmap.width, bitmap.height), ctx = getWorkContext(plan.resizedWidth, plan.resizedHeight);
  ctx.imageSmoothingEnabled = !0, ctx.imageSmoothingQuality = "high", ctx.drawImage(bitmap, 0, 0, plan.resizedWidth, plan.resizedHeight);
  let rgba = ctx.getImageData(plan.left, plan.top, plan.cropSize, plan.cropSize).data;
  return rgbaToImageNetNchw(rgba, plan.cropSize, plan.cropSize);
}
function sigmoid(value) {
  return value >= 0 ? 1 / (1 + Math.exp(-value)) : Math.exp(value) / (1 + Math.exp(value));
}
function aggregateTileProbabilities(probabilities, rule = DEFAULT_TILE_AGGREGATION) {
  ...
  if (rule === TILE_AGGREGATION.TOP2 || rule === TILE_AGGREGATION.TOP3) {
    let k2 = rule === TILE_AGGREGATION.TOP2 ? 2 : 3, top = [...values].sort((a, b) => b - a).slice(0, Math.min(k2, values.length));
...
var MonotoneCalibrator = class _MonotoneCalibrator {
  ...
  constructor(table) {
    ...
    this.xs = Float64Array.from(xs2), this.ys = Float64Array.from(ys2), this.id = table.id ?? "unnamed", this.fittedOn = table.fittedOn ?? null;
  }
  /**
  ...
  apply(raw) {
    ...
    let { xs: xs2, ys: ys2 } = this, n = xs2.length;
    if (raw <= /** @type {number} */
    xs2[0]) return (
      /** @type {number} */
      ys2[0]
...
var NATIVE_VETO_MAX_TAU = 0.1, NATIVE_VETO_OFFICIAL_FLOOR = 0.05, NATIVE_VETO_MIN_SHORT_EDGE = 440;
function shouldProbeNativeTiles(input) {
  return input.graphicGated ? !1 : input.shortEdge > NATIVE_VETO_MIN_SHORT_EDGE && input.officialScore >= NATIVE_VETO_OFFICIAL_FLOOR;
}

// src/offscreen/ort-engine.js
var ALLOWED_AGGREGATION = new Set(Object.values(TILE_AGGREGATION)), wasmConfigured = !1;
function configureWasm() {
  wasmConfigured || (silenceWindowsWebGpuPowerPreference(), Q.wasm.numThreads = 1, Q.wasm.proxy = !1, Q.wasm.wasmPaths = chrome.runtime.getURL("ort/"), Q.logLevel = "error", wasmConfigured = !0);
}
function gpuAvailable() {
  return !!/** @type {{gpu?: unknown} | undefined} */
  globalThis.navigator?.gpu;
}
...
async function createSession(bytes) {
  let wasm = sessionOptions(["wasm"]);
  if (gpuAvailable())
    try {
      return await Xf.create(bytes.slice(0), sessionOptions(["webgpu", "wasm"]));
    } catch (err) {
      console.warn("[ai-image-detector] WebGPU session failed, using WASM:", err);
    }
  return Xf.create(bytes, wasm);
}
async function warmupSession(session, inputName, outputName) {
  let zeros = new Float32Array(3 * 384 * 384), input = new Le("float32", zeros, [1, 3, 384, 384]), output;
  try {
    output = (await session.run({ [inputName]: input }, [outputName]))[outputName];
  } finally {
    input.dispose(), output?.dispose();
  }
}
async function probeNativeVetoTiles(bitmap, officialScore, runSingle, graphicGated) {
  if (!shouldProbeNativeTiles({
    shortEdge: Math.min(bitmap.width, bitmap.height),
    officialScore,
    graphicGated
  }))
    return {};
  let tiles = planNativeTiles(bitmap.width, bitmap.height), cx = bitmap.width / 2, cy = bitmap.height / 2;
  tiles.sort((a, b) => {
    let da = (a.x + a.size / 2 - cx) ** 2 + (a.y + a.size / 2 - cy) ** 2, db = (b.x + b.size / 2 - cx) ** 2 + (b.y + b.size / 2 - cy) ** 2;
    return da - db;
  });
  let probabilities = [];
  for (let tile of tiles) {
    let probability = await runSingle(bitmap, bitmapTileToNchw(bitmap, tile));
    if (probabilities.push(probability), probability >= NATIVE_VETO_MAX_TAU)
      return { nativeMax: probability };
  }
  let ordered = [...probabilities].sort((a, b) => a - b), mid = Math.floor(ordered.length / 2);
  return {
    nativeMax: (
      /** @type {number} */
...
async function createOrtEngine(entry, modelBytes) {
  configureWasm();
  let session = await createSession(modelBytes), hasRun = !1, runTail = Promise.resolve(), teardown = async () => {
    await session.release();
  }, runExclusive = (task) => {
    let next = runTail.then(task, task);
    return runTail = next.then(
      () => {
      },
      () => {
      }
    ), next;
  }, inputName = entry.input?.name ?? session.inputNames[0], outputName = entry.output?.name ?? session.outputNames[0];
  if (!inputName || !session.inputNames.includes(inputName))
    throw await teardown(), new Error(`model input '${inputName ?? "(missing)"}' does not match ONNX graph`);
  if (!outputName || !session.outputNames.includes(outputName))
    throw await teardown(), new Error(`model output '${outputName ?? "(missing)"}' does not match ONNX graph`);
  if (entry.input?.width != null && entry.input.width !== 384)
    throw await teardown(), new Error(`unsupported model input width: ${entry.input.width}`);
  if (entry.input?.height != null && entry.input.height !== 384)
    throw await teardown(), new Error(`unsupported model input height: ${entry.input.height}`);
  if (entry.output?.semantic && entry.output.semantic !== "fake-logit")
    throw await teardown(), new Error(`unsupported model output semantic: ${entry.output.semantic}`);
  let centerCrop = entry.tiling?.mode === "official-center";
  if (entry.tiling?.mode && !centerCrop && entry.tiling.mode !== "native-grid")
    throw await teardown(), new Error(`unsupported production preprocessing mode: ${entry.tiling.mode}`);
  if (!centerCrop && entry.tiling?.nativeResolution === !1)
    throw await teardown(), new Error("whole-image resize is not a supported inference path");
  let aggregation = entry.tiling?.aggregation ?? DEFAULT_TILE_AGGREGATION;
  if (!centerCrop && !ALLOWED_AGGREGATION.has(aggregation))
    throw await teardown(), new Error(`unsupported tile aggregation: ${aggregation}`);
  let runSingle = async (bitmap, data) => runExclusive(async () => {
    let input = new Le("float32", data, [1, 3, 384, 384]), output;
    try {
      output = (await session.run({ [inputName]: input }, [outputName]))[outputName];
      let first = output?.data[0], logit = typeof first == "bigint" ? Number(first) : first;
      if (typeof logit != "number" || !Number.isFinite(logit))
        throw new Error("ONNX detector returned a non-finite logit");
      return hasRun = !0, sigmoid(logit);
    } finally {
      input.dispose(), output?.dispose();
    }
  });
  return {
    async warmup() {
      hasRun || await runExclusive(async () => {
        hasRun || (await warmupSession(session, inputName, outputName), hasRun = !0);
      });
    },
    /**
    ...
    async infer(bitmap, options) {
      if (centerCrop) {
        let raw = await runSingle(bitmap, bitmapOfficialCenterToNchw(bitmap)), native = await probeNativeVetoTiles(bitmap, raw, runSingle, options?.graphicGated);
        return { raw, ...native };
      }
      let tiles = planNativeTiles(bitmap.width, bitmap.height, {
        tileSize: 384,
        maxAxisSamples: entry.tiling?.maxAxisSamples
      }), probabilities = [];
      for (let tile of tiles) {
        let data = bitmapTileToNchw(bitmap, tile);
        probabilities.push(await runSingle(bitmap, data));
      }
      return { raw: aggregateTileProbabilities(probabilities, aggregation) };
    },
    async release() {
      await runTail, await teardown();
    }
  };
}

// src/offscreen/infer-queue.js
function createInferQueue(options) {
  let fetchConcurrency = options.fetchConcurrency, inferConcurrency = options.inferConcurrency ?? 1, priorityOf = options.priorityOf ?? (() => 0), keyOf = options.keyOf;
  ...
  let seq = 0, inferActive = 0, waiting = [], fetching = [], ready = [], byKey = /* @__PURE__ */ new Map();
  function forget(job) {
    job.key !== void 0 && byKey.get(job.key) === job && byKey.delete(job.key);
  }
  ...
  function maxPriority(list) {
    let highest = -1 / 0;
    for (let job of list)
      job.priority > highest && (highest = job.priority);
    return highest;
  }
  function canStartFetch(job) {
    let occupancy = fetching.length + ready.length;
    if (occupancy < fetchConcurrency) return !0;
    let occupiedMax = Math.max(maxPriority(ready), maxPriority(fetching));
    return job.priority > occupiedMax && occupancy < fetchConcurrency + 1;
  }
  function pumpFetches() {
    sortInPlace(waiting);
    let started = !1;
    for (; waiting.length > 0; ) {
      let job = waiting[0];
      if (!job || !canStartFetch(job)) break;
      waiting.shift(), fetching.push(job), started = !0, Promise.resolve().then(() => options.acquire(job.req)).then(
        (acquired) => {
          job.acquired = acquired, finishFetch(job, null);
        },
  ...
  function pumpInfers() {
    sortInPlace(ready);
    let blockedBy = maxPriority(fetching), started = !1;
    for (; inferActive < inferConcurrency && ready.length > 0; ) {
      let next = ready[0];
      if (!next || blockedBy > next.priority) break;
      ready.shift(), inferActive += 1, started = !0;
      let acquired = (
        /** @type {TAcquired} */
        next.acquired
      );
      Promise.resolve().then(() => options.run(next.req, acquired)).then(next.resolve, next.reject).finally(() => {
        forget(next), inferActive -= 1, pump();
      });
    }
    return started;
  }
  function pump() {
    for (; ; ) {
      let fetchStarted = pumpFetches(), inferStarted = pumpInfers();
      if (!fetchStarted && !inferStarted) return;
    }
  }
  function finishFetch(job, err) {
    if (job.dropped) return;
    let idx = fetching.indexOf(job);
    idx >= 0 && fetching.splice(idx, 1), err ? (forget(job), job.reject(err)) : ready.push(job), pump();
  }
  function clear(reason = "cancelled") {
    let err = new Error(reason), drop = (job) => {
      job.dropped = !0, forget(job), job.reject(err);
    };
    for (let job of waiting) drop(job);
    waiting.length = 0;
    for (let job of fetching) drop(job);
    fetching.length = 0;
    for (let job of ready) drop(job);
    ready.length = 0;
  }
  function enqueue(req) {
    let key = keyOf ? keyOf(req) : void 0;
    if (key !== void 0) {
      let existing = byKey.get(key);
      if (existing && !existing.dropped && existing.promise) {
        let nextPriority = priorityOf(req);
        return nextPriority > existing.priority && (existing.priority = nextPriority, existing.req = req, pump()), existing.promise;
      }
    }
    let job = {
      req,
    ...
    return job.promise = promise, job.key !== void 0 && byKey.set(job.key, job), waiting.push(job), pump(), promise;
  }
  function reprioritize(key, priority) {
    if (!Number.isFinite(priority)) return !1;
    let job = byKey.get(key);
    return !job || priority <= job.priority ? !1 : (job.priority = priority, pump(), !0);
  }
...
function createGraphicAccumulator() {
  return { pixels: 0, gradientPixels: 0, flat: 0, soft: 0, hard: 0, maxPatchSoft: 0, colors: /* @__PURE__ */ new Map() };
}
function accumulateGraphicStats(acc, rgba, width, height) {
  let pixels = width * height;
  if (!Number.isInteger(width) || !Number.isInteger(height) || width <= 0 || height <= 0)
    throw new RangeError("patch dimensions must be positive integers");
  if (rgba.length !== pixels * 4)
    throw new RangeError(`expected ${pixels * 4} RGBA bytes, got ${rgba.length}`);
  let luma = new Int32Array(pixels);
  for (let i = 0; i < pixels; i += 1) {
    let o = i * 4, r = rgba[o] ?? 0, g = rgba[o + 1] ?? 0, b = rgba[o + 2] ?? 0;
    luma[i] = 77 * r + 150 * g + 29 * b >> 8;
    let key = r >> 3 << 10 | g >> 3 << 5 | b >> 3, seen = acc.colors.get(key);
    seen !== void 0 ? acc.colors.set(key, seen + 1) : acc.colors.size < 4096 && acc.colors.set(key, 1);
  }
  acc.pixels += pixels;
  let gradientsBefore = acc.gradientPixels, softBefore = acc.soft;
  for (let y = 0; y < height; y += 1) {
    ...
    for (let x = 0; x < width; x += 1) {
      ...
      x + 1 < width && classifyGradient(acc, Math.abs(
        /** @type {number} */
        luma[row + x + 1] - here
      )), y + 1 < height && classifyGradient(acc, Math.abs(
        /** @type {number} */
        luma[row + width + x] - here
      ));
    }
  }
  let patchGradients = acc.gradientPixels - gradientsBefore;
  patchGradients > 0 && (acc.maxPatchSoft = Math.max(acc.maxPatchSoft, (acc.soft - softBefore) / patchGradients));
}
function classifyGradient(acc, magnitude) {
  acc.gradientPixels += 1, magnitude <= 2 ? acc.flat += 1 : magnitude >= 24 ? acc.hard += 1 : acc.soft += 1;
}
function finalizeGraphicStats(acc) {
  let g = acc.gradientPixels, top8 = 0;
  if (acc.pixels > 0 && acc.colors.size > 0) {
    let counts = [...acc.colors.values()].sort((a, b) => b - a);
    for (let i = 0; i < Math.min(8, counts.length); i += 1)
      top8 += /** @type {number} */
      counts[i];
  }
  return {
...
function evaluateGraphicGate(stats) {
  let t = GRAPHIC_GATE_THRESHOLDS, checks = {
    pixels: stats.pixels >= 1024,
  ...
  return checks.pixels && checks.flat && checks.soft && checks.hard && checks.top8 && checks.patch;
}

// src/offscreen/graphic-stats.js
var GRAPHIC_PATCH_SIZE = 96, GRAPHIC_PATCH_GRID = 3;
function planGraphicAxis(length, patch = GRAPHIC_PATCH_SIZE, maxSamples = GRAPHIC_PATCH_GRID) {
  if (!Number.isInteger(length) || length <= 0) throw new RangeError("image edge must be a positive integer");
  if (length <= patch) return [{ origin: 0, size: length }];
  let count = Math.min(maxSamples, Math.ceil(length / patch)), last = length - patch;
  return count === 1 ? [{ origin: Math.round(last / 2), size: patch }] : Array.from({ length: count }, (_, i) => ({
    origin: Math.round(last * i / (count - 1)),
    size: patch
  }));
}
function planGraphicPatches(width, height) {
  let xs2 = planGraphicAxis(width);
  return planGraphicAxis(height).flatMap((y) => xs2.map((x) => ({ x: x.origin, y: y.origin, width: x.size, height: y.size })));
}
var patchCanvas = null, patchCtx = null;
function getPatchContext(width, height) {
  if ((!patchCanvas || !patchCtx || patchCanvas.width !== width || patchCanvas.height !== height) && (patchCanvas = new OffscreenCanvas(width, height), patchCtx = patchCanvas.getContext("2d", { alpha: !1, willReadFrequently: !0 })), !patchCtx) throw new Error("2D canvas unavailable for graphic statistics");
  return patchCtx;
}
function bitmapGraphicStats(bitmap) {
  let acc = createGraphicAccumulator();
  for (let patch of planGraphicPatches(bitmap.width, bitmap.height)) {
    let ctx = getPatchContext(patch.width, patch.height);
    ctx.fillStyle = "#ffffff", ctx.fillRect(0, 0, patch.width, patch.height), ctx.drawImage(bitmap, patch.x, patch.y, patch.width, patch.height, 0, 0, patch.width, patch.height);
    let rgba = ctx.getImageData(0, 0, patch.width, patch.height).data;
    accumulateGraphicStats(acc, rgba, patch.width, patch.height);
  }
  let stats = finalizeGraphicStats(acc), gated = evaluateGraphicGate(stats);
  return { ...stats, gated };
}

// src/offscreen/offscreen.js
var manifestEntries = null, modelStatus = { state: "missing" }, enginePromise = null, downloadPromise = null, runtimeInitFailed = !1;
async function loadManifest() {
  return manifestEntries || (manifestEntries = /** @type {ModelManifestEntry[]} */
  ((await (await fetch(chrome.runtime.getURL("models/manifest.json"))).json()).models ?? []).map(
    (entry) => entry.bundledPath ? { ...entry, url: chrome.runtime.getURL(entry.bundledPath) } : entry
  ), /** @type {ModelManifestEntry[]} */
  manifestEntries);
}
async function refreshModelStatus() {
  if (downloadPromise) return modelStatus;
  let primary = (await loadManifest())[0];
  if (!primary || !primary.url || !primary.sha256)
    return modelStatus = {
      state: "not-configured",
      detail: "the verified model artifact has no public setup URL yet. Build with npm run build:local-model to bundle the local weights."
    }, modelStatus;
  let file = await getInstalledModel(primary);
  if (downloadPromise) return modelStatus;
  let progress = file ? 0 : await getPartialDownloadProgress(primary);
  return downloadPromise || (modelStatus = file ? { state: "ready", modelId: primary.id, modelSha256: primary.sha256 ?? void 0 } : { state: "missing", ...progress > 0 ? { progress } : {} }), modelStatus;
}
async function runDownload() {
  ...
  try {
    ...
    return progress = await getPartialDownloadProgress(primary), modelStatus = { state: "downloading", progress }, await downloadAndInstall(primary, (nextProgress) => {
      progress = Math.max(progress, nextProgress), modelStatus = { state: "downloading", progress }, chrome.runtime.sendMessage({ type: MSG.MODEL_PROGRESS, target: TARGET.SW, progress }).catch(() => {
      });
    }), enginePromise = null, runtimeInitFailed = !1, modelStatus = { state: "ready", modelId: primary.id, modelSha256: primary.sha256 ?? void 0 }, scheduleEngineWarmup(), modelStatus;
  } catch (err) {
    return modelStatus = {
      state: "error",
...
function startDownload() {
  return downloadPromise || (downloadPromise = runDownload().finally(() => {
    downloadPromise = null;
  }), downloadPromise);
}
function currentModelStatus() {
  return downloadPromise || runtimeInitFailed && modelStatus.state === "error" ? Promise.resolve(modelStatus) : refreshModelStatus();
}
async function readCapped(res, limit) {
  let declared = Number(res.headers.get("content-length"));
  ...
  let reader = res.body.getReader(), chunks = [], total = 0;
  try {
    for (; ; ) {
      let { done, value } = await reader.read();
      if (done) break;
      if (value) {
        if (total += value.byteLength, total > limit) throw new Error(`image too large: exceeded ${limit} bytes`);
        chunks.push(value);
      }
    }
  } finally {
  ...
  let bytes = new ArrayBuffer(total), view = new Uint8Array(bytes), offset = 0;
  for (let chunk of chunks)
    view.set(chunk, offset), offset += chunk.byteLength;
  return bytes;
}
async function fetchImageBytes(url) {
  ...
  let controller = new AbortController(), timer = setTimeout(() => controller.abort(new Error("image fetch timed out")), 1e4);
  try {
    ...
    return { bytes: await readCapped(res, 31457280), type: res.headers.get("content-type") ?? "" };
  } finally {
    clearTimeout(timer);
  }
}
function ensureEngine() {
  return enginePromise || (enginePromise = (async () => {
    let primary = (await loadManifest())[0];
    if (!primary) throw new Error("empty models/manifest.json");
    let bytes = await getInstalledModelBytes(primary);
...
function scheduleEngineWarmup() {
  chrome.runtime?.id && ensureEngine().then((engine) => {
    setTimeout(() => {
      engine.warmup().catch(markRuntimeInitializationError);
    }, 150);
  }).catch(markRuntimeInitializationError);
}
function markRuntimeInitializationError(err) {
  runtimeInitFailed = !0, modelStatus = {
    state: "error",
    detail: `runtime initialization failed: ${err instanceof Error ? err.message : String(err)}`
  };
}
async function inferOrt(bitmap, options) {
  return (await ensureEngine()).infer(bitmap, options);
}
var inferQueue = createInferQueue({
  fetchConcurrency: 4,
  inferConcurrency: 1,
...
function enqueueInfer(req) {
  return inferQueue.enqueue(req).catch((err) => ({ id: req.id, ok: !1, error: String(err) }));
}
function withDeadline(promise, ms2, controller) {
  let timer, deadline = new Promise((_resolve, reject) => {
    timer = setTimeout(() => {
      let err = new Error(`scan job exceeded ${ms2}ms`);
      controller.abort(err), reject(err);
    }, ms2);
  });
  return Promise.race([
    promise,
...
async function runInferFromFetched(req, fetched) {
  ...
  return withDeadline(runInferInner(req, fetched, controller.signal), 3e4, controller).catch(
    (err) => ({ id: req.id, ok: !1, error: String(err) })
  );
}
async function runInferInner(req, fetched, signal) {
  ...
  try {
    ...
    let declaredType = fetched.type.split(";")[0]?.trim().toLowerCase() ?? "";
    ...
    let blob = new Blob([fetched.bytes], { type: fetched.type }), bitmap = await createImageBitmap(blob, { imageOrientation: "from-image" });
    try {
      if (bitmap.width * bitmap.height > 1e8)
        return {
          id: req.id,
          ok: !1,
          error: `image too large decoded: ${bitmap.width}x${bitmap.height} exceeds ${1e8} pixels`
        };
      let hashPromise = sha256Hex(fetched.bytes), graphic = bitmapGraphicStats(bitmap);
      if (req.statsOnly)
        return {
          id: req.id,
          ok: !0,
      ...
      let status = modelStatus.state === "ready" ? modelStatus : await currentModelStatus();
      if (status.state === "ready") {
        let [inferred, hash2] = await Promise.all([
      ...
      return { id: req.id, ok: !1, error: "no-model" };
    } finally {
      bitmap.close();
    }
  } catch (err) {
    return { id: req.id, ok: !1, error: String(err) };
  }
}
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (!msg || msg.target !== TARGET.OFFSCREEN) return !1;
  switch (msg.type) {
    case MSG.INFER_RUN:
      return enqueueInfer(msg).then(sendResponse), !0;
    case MSG.INFER_PRIORITY:
      return sendResponse(inferQueue.reprioritize(msg.id, msg.priority)), !1;
    case MSG.INFER_CANCEL:
      return inferQueue.clear("disabled"), sendResponse(!0), !1;
    case MSG.MODEL_STATUS_GET:
      return currentModelStatus().catch((err) => (
        /** @type {ModelStatus} */
        { state: "error", detail: String(err) }
      )).then(sendResponse), !0;
    case MSG.MODEL_DOWNLOAD:
      return startDownload().then(sendResponse), !0;
    default:
      return !1;
  }
});
refreshModelStatus().then((status) => {
  status.state === "ready" && scheduleEngineWarmup();
}).catch(() => {
});
